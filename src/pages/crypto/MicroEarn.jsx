// src/pages/crypto/MicroEarn.jsx
...
import {
  FaWallet, FaMedal, FaTrophy, FaBuilding, FaCoins
} from 'react-icons/fa';
...
// Stores coin quantities per platform. Live USD/INR values are computed in UI.
const syncMicroEarnSummary = async (userId) => {
  try {
    const snap = await getDocs(collection(db, 'users', userId, 'microEarnLogs'));
    ...
    logs.forEach(t => {
      const plat = t.platform?.trim() || 'Unknown Platform';
      existingSources.add(plat);
      if (t.destinationWallet) existingWallets.add(t.destinationWallet.trim());

      if (!stats[plat]) stats[plat] = { platform: plat, withdrawCount: 0, coins: new Set(), coinQuantities: {} };
      
      const rQty = parseFloat(t.receivedAmount) || 0;
      const coinUpper = (t.coin || 'USDT').toUpperCase();
      
      stats[plat].withdrawCount += 1;
      stats[plat].coins.add(coinUpper);
      stats[plat].coinQuantities[coinUpper] = (stats[plat].coinQuantities[coinUpper] || 0) + rQty;
    });

    const platformsArray = Object.values(stats).map(s => ({ ...s, coins: Array.from(s.coins) }));

    const summaryData = {
      platforms: platformsArray,
      existingSources: Array.from(existingSources),
      existingWallets: Array.from(existingWallets),
      totalLogs: logs.length,
      lastUpdated: new Date().toISOString()
    };

    await setDoc(doc(db, 'users', userId, 'walletSummary', 'microEarn'), summaryData, { merge: true });
    return summaryData;
  } catch (error) {
    console.error("Failed to sync micro earn summary:", error);
    return null;
  }
};
      ...
      <div className="relative z-10 flex-1 flex flex-col">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3.5 mb-4 border border-slate-100 dark:border-slate-700/50 flex justify-between items-center gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Live Yield</p>
...
// ============================================
const MicroEarnContent = () => {
  const { user, baseCurrency = 'INR', selectedCryptos = [], formatGlobalDate, getCalendarMonthKey } = useAuth();
  ...
  const [globalSummary, setGlobalSummary] = useState({
    platforms: [], existingSources: [], existingWallets: [], totalLogs: 0
  });

  const summaryQuery = useMemo(() => {
    if (!user?.uid) return null;
    return query(collection(db, 'users', user.uid, 'walletSummary'), where(documentId(), '==', 'microEarn'));
  }, [user?.uid]);
  ...
  useEffect(() => {
    ...
    if (summaryDataArray && summaryDataArray.length > 0) {
      ...
      syncMicroEarnSummary(user.uid).then(data => {
  ...
  } = usePaginatedMonthlyLogs(user?.uid, 'microEarnLogs', 20, !!user, getCalendarMonthKey);
  ...
  // ✅ Simple live price filler for form – Context based
  const fillLivePriceForForm = () => {
    if (!formData.coin) return;
    const priceUSD = livePrices[formData.coin.toUpperCase()]?.priceUSD;
    if (priceUSD) {
      const priceBase = priceUSD * fiatRate;
      setFormData(prev => ({ ...prev, entryPrice: priceBase.toFixed(6).replace(/\.?0+$/, '') }));
    } else {
      addToast("Live price not available for this coin yet.", "warning");
    }
  };

  // 🚀 FIXED RANK STABILITY (Calculates live value dynamically from summary quantities)
  const platformRankings = useMemo(() => {
    if (!globalSummary.platforms) return [];
    const ranked = globalSummary.platforms.map(stat => {
      let val = 0;
      Object.entries(stat.coinQuantities || {}).forEach(([coinSymbol, qty]) => {
        val += ((parseFloat(qty) || 0) * (livePrices[coinSymbol]?.priceUSD || 0) * fiatRate);
      });
      return { ...stat, totalLiveValue: val };
    });

    return ranked.sort((a, b) => {
      const diff = b.totalLiveValue - a.totalLiveValue;
      if (diff !== 0) return diff;
      return a.platform.localeCompare(b.platform);
    }).map((stat, idx) => ({ ...stat, rank: idx + 1 }));
  }, [globalSummary.platforms, livePrices, fiatRate]);

  const totalVaultValueBase = platformRankings.reduce((total, stat) => total + stat.totalLiveValue, 0);

  // 🚀 3. LOCAL UI FILTERING FOR LEDGER
  const filteredLedger = useMemo(() => {
    if (!paginatedMonths) return [];
    
    // Process previous dates across loaded records to approximate "Days Since Last"
    const loadedRecords = paginatedMonths.flatMap(m => m.records).sort((a, b) => new Date(a.date) - new Date(b.date));
    const platformLastDateMap = {};
    
    const enrichedRecords = loadedRecords.map(t => {
      const plat = t.platform;
      let daysSinceLast = null;
      if (platformLastDateMap[plat]) {
        daysSinceLast = Math.ceil(Math.abs(new Date(t.date) - new Date(platformLastDateMap[plat])) / (1000 * 60 * 60 * 24));
      }
      platformLastDateMap[plat] = t.date;
      ...
      return { ...t, daysSinceLast, currentLiveBaseAmount };
    }).reverse(); // Latest first again
    ...
    enrichedRecords.forEach(r => {
      ...
      const mKey = getCalendarMonthKey ? getCalendarMonthKey(r.date) : `${dateObj.getFullYear()}-${String(dateObj.getMonth()+1).padStart(2,'0')}`;
      
      if (!newMonthsMap[mKey]) {
        newMonthsMap[mKey] = {
          monthKey: mKey,
      ...
      newMonthsMap[mKey].records.push(r);
      newMonthsMap[mKey].totalInValue += (r.currentLiveBaseAmount || 0);
    });

    return Object.values(newMonthsMap).sort((a, b) => b.monthKey.localeCompare(a.monthKey));

  }, [paginatedMonths, searchTerm, livePrices, fiatRate, formatGlobalDate, getCalendarMonthKey]);
  ...
  const handleSaveEntry = async (e) => {
    ...
    if (wQty <= 0 || rQty <= 0) { addToast("Amounts must be greater than zero.", "warning"); return; }
    if (!formData.platform.trim()) { addToast("Please enter a website/platform name!", "warning"); return; }
    if (!formData.destinationWallet.trim()) { addToast("Please specify the Destination Wallet!", "warning"); return; }
    if (formData.methods.length === 0) { addToast("Please select at least one earning method.", "warning"); return; }

    setIsSaving(true);

    let editDateStr = formData.date;
    const oldRec = editingId ? paginatedMonths.flatMap(m => m.records).find(t => t.id === editingId) : null;
    const timestamp = oldRec?.timestamp || new Date(editDateStr).getTime();
    ...
    try {
      if (editingId) {
        ...
        addToast('Withdrawal logged & synced!', 'success');
      }
      
      // Resync global summary
      syncMicroEarnSummary(user.uid).then(data => { if (data) setGlobalSummary(data); });
      refreshTransactions();
      closeModal();
    } catch (error) { addToast("Failed to save transaction.", "error"); } finally { setIsSaving(false); }
  };
  ...
  const openModal = () => {
    setEditingId(null);
    setFormData({ 
      platform: globalSummary.existingSources?.[0] || '', 
      methods: [], 
      destinationWallet: globalSummary.existingWallets?.[0] || '', 
      coin: availableCryptos[0] || 'USDT', 
      withdrawnAmount: '', 
      receivedAmount: '', 
  ...
  const SkeletonLeaderboard = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
      {[1,2,3].map(i => (
        <div key={i} className="rounded-[1.5rem] p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm animate-pulse space-y-4">
    ...
    <div className="space-y-6">
      {[1,2].map(i => (
        <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[2rem] overflow-hidden shadow-sm animate-pulse">
          <div className="px-6 py-5 flex justify-between items-center bg-slate-50/80 dark:bg-slate-800/50">
      ...
      <div className="pt-8 md:pt-12 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto px-4 md:px-6">
              ...
              <div className="relative w-full md:w-auto">
                ...
                {isExportMenuOpen && (
                  <div className="absolute top-[110%] right-0 w-full md:w-40 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl flex flex-col p-1.5 z-[100] animate-in fade-in zoom-in-95">
                    <button onMouseDown={(e) => { e.preventDefault(); handleDownloadReport('pdf'); }} className="flex items-center gap-2.5 px-4 py-3 hover:bg-slate-700 text-slate-200 text-[11px] font-black rounded-lg transition-colors"><HiOutlineDocumentText className="text-rose-400" size={18}/> PDF Document</button>
                    <button onMouseDown={(e) => { e.preventDefault(); handleDownloadReport('excel'); }} className="flex items-center gap-2.5 px-4 py-3 hover:bg-slate-700 text-slate-200 text-[11px] font-black rounded-lg transition-colors"><HiOutlineTable className="text-emerald-400" size={18}/> Excel (CSV)</button>
                  </div>
                )}
              </div>
        ...
        {isLoadingLogs ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-lg font-black text-slate-700 dark:text-slate-300">No withdrawal logs found</p>
            <p className="text-sm text-slate-500">{searchTerm ? 'Adjust your search.' : 'Start by logging your first withdrawal.'}</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredLedger.map((month) => {
              const isOpen = openMonths.has(month.monthKey);
              const totalValue = month.totalInValue;
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right hidden sm:block">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Total Yield (Page)</p>
                        <p className="text-sm font-black text-emerald-600 dark:text-emerald-400">{currencySymbol}{totalValue.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                      </div>
                            ...
                            <div key={rec.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                              ...
                              <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                                ...
                                <div>
                                  ...
                                  <p className="font-black text-slate-800 dark:text-white">{currencySymbol}{(rec.currentLiveBaseAmount || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                                </div>
                                <div>
                            ...
                            {month.records.map((rec) => {
                              ...
                              return (
                                <tr key={rec.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group">
                                  ...
                                  <td className="p-4 text-right"><p className="text-base font-black text-slate-800 dark:text-white truncate max-w-[150px] ml-auto">{currencySymbol}{(rec.currentLiveBaseAmount || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}</p></td>
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1 space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative mt-1">
                   ...
                   <input type="text" list="micro-sources" required autoFocus={!editingId} value={formData.platform} onChange={(e) => setFormData({...formData, platform: e.target.value})} placeholder="e.g. FreeBitcoin, Cointiply" className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-amber-500/50 shadow-sm transition-colors placeholder-slate-400" />
                   <datalist id="micro-sources">{globalSummary.existingSources.map(p => <option key={p} value={p}>{p}</option>)}</datalist>
                </div>
              </div>
              <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative mt-1">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 p-1 flex items-center justify-center overflow-hidden z-10 pointer-events-none"><LogoRenderer symbol={formData.coin} logoUrl={fullDatabase.find(c=>c.symbol===formData.coin)?.logo} bg={fullDatabase.find(c=>c.symbol===formData.coin)?.bg} color={fullDatabase.find(c=>c.symbol===formData.coin)?.color} /></div>
                    <select required disabled={editingId} value={formData.coin} onChange={(e) => setFormData({...formData, coin: e.target.value})} className="w-full pl-14 pr-10 py-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-black text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-amber-500/50 appearance-none shadow-sm cursor-pointer disabled:opacity-50 transition-colors">
                      {availableCryptos.length > 0 ? availableCryptos.map(c => { const dbCoin = fullDatabase.find(x => x.symbol === c) || {}; return <option key={c} value={c}>{dbCoin.name || c} ({c})</option>; }) : <option value="USDT">USDT (Default)</option>}
                    </select>
                ...
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <input type="text" list="micro-wallets" required value={formData.destinationWallet} onChange={(e) => setFormData({...formData, destinationWallet: e.target.value})} placeholder="e.g. FaucetPay, Binance" className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-amber-500/50 shadow-sm transition-colors placeholder-slate-400" />
                    <datalist id="micro-wallets">{globalSummary.existingWallets.map(w => <option key={w} value={w}>{w}</option>)}</datalist>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/50 rounded-2xl space-y-3 shadow-sm">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={fillLivePriceForForm} className="text-[10px] font-black bg-blue-600 text-white px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-blue-700 transition-colors shadow-sm uppercase tracking-widest"><HiOutlineRefresh size={14} /> Fetch Live</button>
                </div>
              ...
              <div>
                <label className="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest ml-1 flex justify-between"><span>Date & Time</span><span className="text-amber-600 dark:text-amber-400">{formatGlobalDate && formData.date ? formatGlobalDate(formData.date.split('T')[0], 'short') : ''}</span></label>
...
export default MicroEarnContent;