...
const MicroEarnContent = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      addToast('Failed to load micro earn logs.', 'error');
      setIsLoading(false);
    });
    return () => unsubscribe();
  }, [user, addToast]);
  ...
  // ❌ REMOVED: fetchMarketData, fetchLivePriceForForm, and related effects

  // ✅ Simple live price filler for form – context से कीमत लेता है
  ...
  // 🚀 FIXED RANK STABILITY (tie-break by platform name)
  const platformRankings = useMemo(() => {
    ...
    Object.values(stats).forEach(stat => {
      ...
      Object.entries(stat.coinQuantities).forEach(([coinSymbol, qty]) => {
        val += (qty * (livePrices[coinSymbol]?.priceUSD || 0) * fiatRate);
      });
      stat.totalLiveValue = val;
    });

    const sorted = Object.values(stats).sort((a, b) => {
      const valueDiff = b.totalLiveValue - a.totalLiveValue;
  ...
  const processedMonths = useMemo(() => {
    ...
    sorted.forEach(t => {
      ...
      const value = t.currentLiveBaseAmount || 0;
      grouped[monthKey].records.push(t);
      grouped[monthKey].totalInValue += value;
    });
  ...
  return (
    <div className="h-full min-h-screen overflow-y-auto pb-24">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right hidden sm:block">
                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Total Yield</p>
      ...
      {/* Modal (unchanged, except live price button now uses context) */}
      {isModalOpen && (
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1 space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative mt-1">
                   ...
                   <datalist id="micro-sources">{existingSources.map(p => <option key={p} value={p}>{p}</option>)}</datalist>
                </div>
              </div>
              <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <datalist id="micro-wallets">{existingWallets.map(w => <option key={w} value={w}>{w}</option>)}</datalist>
                  </div>
                </div>
              </div>
