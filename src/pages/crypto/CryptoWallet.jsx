...
import {
  ...
  query, orderBy, getDoc, getDocs, where, documentId
...
// ─── ENTERPRISE ENGINE: Syncs perfect math to a summary document ─────────────

const syncCryptoSummary = async (userId) => {
  try {
    const snap = await getDocs(collection(db, 'users', userId, 'cryptoWalletLogs'));
    ...
    logs.forEach(t => {
      ...
      if (t.platform) existingPlatforms.add(t.platform);
      if (t.fromPlatform) existingPlatforms.add(t.fromPlatform);
      if (t.toPlatform) existingPlatforms.add(t.toPlatform);
    ...
    await setDoc(doc(db, 'users', userId, 'walletSummary', 'crypto'), summaryData, { merge: true });
    return summaryData;
  } catch (error) {
    console.error("Failed to sync crypto summary:", error);
    return null;
  }
};
...
const CryptoWalletContent = () => {
  ...
  const [globalSummary, setGlobalSummary] = useState({
    holdings: {}, existingPlatforms: [], totalLogs: 0
  });
  ...
  useEffect(() => {
    ...
    if (summaryDataArray && summaryDataArray.length > 0) {
      ...
      syncCryptoSummary(user.uid).then(data => {
  ...
  } = usePaginatedMonthlyLogs(user?.uid, 'cryptoWalletLogs', 20, !!user, getCalendarMonthKey);
  ...
  useEffect(() => {
    ...
    const fetchVaults = async () => {
      // Try walletSummary first (faster), fallback to full scan
      ...
      const names = new Set();
      if (bankSum.exists() && bankSum.data().existingBanks)
        bankSum.data().existingBanks.forEach(n => names.add(n));
      if (onlineSum.exists() && onlineSum.data().existingWallets)
        onlineSum.data().existingWallets.forEach(n => names.add(n));

      // Fallback: scan raw collections if summaries not yet built
      if (names.size === 0) {
  ...
  useEffect(() => {
    if (!user) return;
    // Platforms come from globalSummary once loaded
    if (globalSummary.existingPlatforms?.length > 0) {
      setExistingCryptoPlatforms(globalSummary.existingPlatforms);
    }
  }, [user, globalSummary.existingPlatforms]);
  ...
  // ─── From globalSummary ──────────────────────────────────────────────────
  const holdings = globalSummary.holdings || {};
  ...
  const filteredLedger = useMemo(() => {
    ...
    return paginatedMonths.map(month => {
      ...
      let pageInQty = 0, pageOutQty = 0;
      filteredRecords.forEach(r => {
        const qty = parseFloat(r.quantity) || 0;
        if (r.type === 'in') pageInQty += qty;
        else if (r.type === 'out') pageOutQty += qty;
      });
      return {
        ...month,
        monthName: localizedMonthName,
        records: filteredRecords,
        totalInQty: pageInQty,
        totalOutQty: pageOutQty
      };
  ...
  const handleSaveEntry = async (e) => {
    ...
    const timestamp = oldRec?.timestamp || new Date(formData.date).getTime();
    ...
    try {
      ...
      // Resync summary after any write
      syncCryptoSummary(user.uid).then(data => { if (data) setGlobalSummary(data); });
      refreshTransactions();
      closeModal();
    } catch (error) {
  ...
  const handleEdit = (rec) => {
    ...
    // ✅ FIX: Set transactionType from the record itself, not from the toggle
    ...
    setFormData({
      coin: rec.coin, quantity: rec.quantity,
      platform: rec.platform || rec.fromPlatform || existingCryptoPlatforms[0] || '',
  ...
  const handleDownloadReport = (format) => {
    ...
    const allRecords = (paginatedMonths || []).flatMap(m => m.records);
    if (allRecords.length === 0) { addToast("No records found.", "warning"); return; }

    const reportData = allRecords.map(rec => ({
    ...
    <div className="h-full min-h-screen overflow-y-auto pb-24">
      <div className="pt-8 md:pt-12 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div className="relative rounded-[2.5rem] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 md:p-8 shadow-2xl border border-slate-700/50 z-20">
          ...
          <div className="relative z-50 flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full md:w-auto mt-4 md:mt-0">
                ...
                {isExportMenuOpen && (
                  <div className="absolute top-[110%] right-0 md:left-0 w-full md:w-40 bg-slate-800 border border-slate-700 rounded-xl shadow-xl flex flex-col p-1.5 animate-in fade-in zoom-in-95 z-[9999]">
                    <button onMouseDown={(e) => { e.preventDefault(); handleDownloadReport('pdf'); }} className="flex items-center gap-2 px-3 py-2.5 hover:bg-slate-700 text-slate-300 text-[10px] font-black rounded-lg transition-colors text-left">
                      <HiOutlineDocumentText className="text-rose-400" size={16} /> PDF Report
                    </button>
              ...
              <button onClick={() => openModal('out')} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white px-5 py-3.5 rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest transition-all border border-white/10 shadow-sm">
                <FaArrowUp size={14} /> Sell / Out
              </button>
              <button onClick={() => openModal('in')} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 text-white px-6 py-3.5 rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest shadow-lg shadow-orange-500/30 transition-all active:scale-95">
                <FaArrowDown size={14} /> Buy / Deposit
              </button>
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-2 md:grid-cols-3 gap-3 mt-5">
            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10 col-span-2 md:col-span-1 overflow-hidden">
              ...
              <p className="text-2xl md:text-xl lg:text-2xl font-black text-white truncate" title={`${currencySymbol}${totalVaultValue.toLocaleString()}`}>
                {currencySymbol}{totalVaultValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </p>
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-3 border border-white/10 overflow-hidden">
              ...
              <p className="text-lg md:text-xl font-black text-white truncate">{globalSummary.totalLogs || 0}</p>
            </div>
          </div>
        </div>

        {/* ── Platform Storage Breakdown ── */}
        <div>
          <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
            <FaChartPie className="text-orange-500" /> Platform Storage Breakdown
          </h3>
          {Object.keys(holdings).length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {Object.entries(holdings)
                .sort((a, b) => b[1].total * (livePrices[b[0].toUpperCase()]?.priceUSD || 0) - a[1].total * (livePrices[a[0].toUpperCase()]?.priceUSD || 0))
                .map(([coin, data]) => {
                    ...
                    <div key={coin} className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-sm hover:shadow-md hover:border-orange-500/30 transition-all group flex flex-col h-full">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3.5 mb-4 border border-slate-100 dark:border-slate-700 flex justify-between items-center gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-sm sm:text-base font-black text-slate-800 dark:text-white truncate" title={data.total}>
                            {data.total % 1 !== 0 ? data.total.toFixed(6).replace(/\.?0+$/, '') : data.total}{' '}
                      ...
                      <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-800">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                          <FaBuilding className="shrink-0" /> Storage Locations
                        </p>
        ...
        <div className="relative mt-8">
          ...
          <input
            type="text"
            placeholder="Search transactions by coin, reason or platform..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-900 border-2 border-slate-300 dark:border-slate-700 rounded-xl font-bold dark:text-white outline-none focus:border-orange-500 transition-all shadow-sm placeholder-slate-400 dark:placeholder-slate-500"
          />
        </div>

        {/* ── Collapsible Monthly Ledger ── */}
        {isLoadingLogs ? (
          <MonthSkeleton />
                              ...
                              <div className="flex flex-wrap justify-between items-end gap-2 mt-3">
                                <div className="flex flex-col gap-1.5">
                                  ...
                                  <p className="text-[9px] font-medium text-slate-400 mt-0.5 flex items-center gap-1">
                                    <HiOutlineCalendar size={10} /> {formatGlobalDate ? formatGlobalDate(rec.date, 'short') : rec.date}
                                  </p>
                                </div>
                                <div className="flex gap-1 items-center">
                                  {(rec.linkedIncomeId || rec.linkedExpenseId || rec.shiftId || rec.linkedPartyId) && (
                                    <span className="px-1.5 py-0.5 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-[8px] font-black rounded border border-amber-300 dark:border-amber-500/30 shadow-sm mr-1">
                                      <FaLock className="inline mb-0.5 mr-0.5" />SYNCED
                                    </span>
                                  )}
                      ...
                      <div className="hidden md:block overflow-x-auto custom-scrollbar">
                        <table className="w-full text-left min-w-[800px]">
                          <thead className="bg-white dark:bg-slate-900 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-200 dark:border-slate-800">
                            <tr>
                              <th className="p-4 pl-6 whitespace-nowrap">Type</th>
                              <th className="p-4 whitespace-nowrap">Asset &amp; Event</th>
                              <th className="p-4 whitespace-nowrap">Platform</th>
                              ...
                              return (
                                <tr key={rec.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group ${rec.linkedIncomeId || rec.linkedExpenseId || rec.shiftId || rec.linkedPartyId ? 'bg-amber-50/20 dark:bg-amber-900/5' : ''}`}>
                                  <td className="p-4 pl-6">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm border ${rec.type === 'in' ? 'bg-emerald-50 border-emerald-200 text-emerald-600 dark:bg-emerald-500/10 dark:border-emerald-500/20 dark:text-emerald-400' : rec.type === 'out' ? 'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-500/10 dark:border-rose-500/20 dark:text-rose-400' : 'bg-blue-50 border-blue-200 text-blue-600 dark:bg-blue-500/10 dark:border-blue-500/20 dark:text-blue-400'}`}>
                                      {rec.type === 'in' ? <HiOutlineDownload size={16} /> : rec.type === 'out' ? <HiOutlineUpload size={16} /> : <HiOutlineSwitchHorizontal size={16} />}
                                    </div>
                                  </td>
                                  <td className="p-4 min-w-[200px]">
                                    <div className="flex items-center gap-3">
                                      <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 p-0.5 border border-slate-200 dark:border-slate-700 shadow-sm shrink-0">
                                        <LogoRenderer symbol={rec.coin} logoUrl={dbCoin.logo} bg={dbCoin.bg} color={dbCoin.color} />
                                      </div>
                                      <div className="min-w-0">
                                        <p className="font-black text-slate-900 dark:text-white text-sm uppercase truncate">{rec.coin}</p>
                                        <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 truncate">{rec.reason}</p>
                                        <p className="text-[9px] font-medium text-slate-400 mt-0.5">{formatGlobalDate ? formatGlobalDate(rec.date, 'short') : rec.date}</p>
                                      </div>
                                    </div>
                                  </td>
                                  <td className="p-4">
                                    <span className="text-[10px] font-black text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700 whitespace-nowrap shadow-sm">
                                      {rec.type === 'transfer' ? `${rec.fromPlatform} → ${rec.toPlatform}` : rec.platform}
                                    </span>
                                  </td>
                                  <td className="p-4 text-right">
                                    <p className={`text-base font-black truncate max-w-[150px] ml-auto ${rec.type === 'in' ? 'text-emerald-600 dark:text-emerald-400' : rec.type === 'out' ? 'text-rose-600 dark:text-rose-400' : 'text-blue-600 dark:text-blue-400'}`}>
                                      {rec.type === 'in' ? '+' : rec.type === 'out' ? '-' : '⇄'}{rec.quantity}
                                    </p>
                                  </td>
      ...
      {/* ════════════════════════════════════════
          ...
          ✅ FIX: No redundant In/Out toggle — type is set by the button that opens the modal
          ════════════════════════════════════════ */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[400] bg-slate-950/80 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200 pt-[60px] md:pt-0">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden max-h-[calc(100dvh-4rem)] sm:max-h-[85vh] border border-slate-300 dark:border-slate-700 animate-in slide-in-from-bottom-10 sm:zoom-in-95">
            ...
            {/* Modal Header — colour reflects type */}
            <div className={`px-6 py-5 flex justify-between items-center text-white shrink-0 ${transactionType === 'in' ? 'bg-gradient-to-r from-orange-500 to-amber-600' : 'bg-gradient-to-r from-rose-600 to-pink-600'}`}>
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1 space-y-6">
              ...
              {formData.isSynced && (
                <div className="bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 p-4 rounded-2xl text-xs font-bold leading-relaxed border border-amber-300 dark:border-amber-500/30 shadow-sm">
                  ...
                  This entry is linked to an external module (like Income Streams). To maintain ledger accuracy, you can only update the{' '}
                  <span className="underline decoration-amber-400">Platform/Storage</span> and{' '}
                  <span className="underline decoration-amber-400">Notes</span> here. To change the quantity or asset, edit the source log.
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <select
                      required
                      disabled={editingId || formData.isSynced}
                      value={formData.coin}
                      onChange={(e) => setFormData({ ...formData, coin: e.target.value })}
                      className="w-full pl-14 pr-10 py-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-black text-slate-900 dark:text-white outline-none focus:border-orange-500 transition-colors appearance-none shadow-sm disabled:opacity-50 cursor-pointer"
                    >
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest ml-1 flex justify-between">
                    ...
                    {transactionType === 'out' && !editingId && (
                      <span className="text-rose-500">Max: {holdings[formData.coin]?.platforms[formData.platform] || 0}</span>
                    )}
                  </label>
                  <input
                    type="number" step="any" required disabled={formData.isSynced}
                    value={formData.quantity}
                    onChange={(e) => setFormData({ ...formData, quantity: e.target.value })}
                    placeholder="e.g. 100"
                    className="w-full mt-1 p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-black text-2xl text-slate-900 dark:text-white outline-none focus:border-orange-500 transition-colors shadow-sm placeholder-slate-400 dark:placeholder-slate-500 disabled:opacity-50"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    type="text" list="crypto-platforms-list" required
                    value={formData.platform}
                    onChange={(e) => setFormData({ ...formData, platform: e.target.value })}
                    placeholder="Type Platform Name (e.g., Phantom)"
                    className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl font-bold text-slate-900 dark:text-white outline-none shadow-sm transition-colors focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50"
                  />
              ...
              {/* Fiat Bridge (Sell only) */}
              {transactionType === 'out' && !editingId && !formData.isSynced && (
                <div className="p-4 sm:p-5 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-300 dark:border-amber-700/50 shadow-sm">
                    ...
                    <div className="mt-5 pt-5 border-t border-amber-200 dark:border-amber-800/50 space-y-5 animate-in fade-in zoom-in-95">
                      <div className="flex flex-wrap sm:flex-nowrap gap-4">
                        <div className="w-full sm:w-[35%] relative">
                          <select
                            ...
                            onChange={(e) => { setFormData({ ...formData, fiatCurrency: e.target.value }); if (e.target.value !== baseCurrency) fetchFiatLiveRate(); }}
                            className="w-full p-4 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-700/50 rounded-xl font-bold text-slate-900 dark:text-white text-sm shadow-sm cursor-pointer outline-none transition-colors appearance-none focus:border-amber-500"
                          >
                        ...
                        <input
                          type="number" step="any" required={isBridging}
                          value={formData.fiatAmount}
                          onChange={(e) => setFormData({ ...formData, fiatAmount: e.target.value })}
                          placeholder="Gross Fiat Received"
                          className="w-full sm:w-[65%] p-4 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-700/50 rounded-xl font-black text-amber-700 dark:text-amber-400 outline-none shadow-sm placeholder-slate-400 transition-colors focus:border-amber-500 focus:ring-2 focus:ring-amber-500/50"
                        />
                      </div>
                        ...
                        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 bg-white dark:bg-slate-900 p-3 sm:p-4 rounded-xl border border-amber-300 dark:border-amber-700/50 shadow-sm">
                          ...
                          <div className="flex items-center gap-2 flex-1 min-w-[150px]">
                            ...
                            <input
                              ...
                              value={formData.fiatExchangeRate}
                              onChange={(e) => setFormData({ ...formData, fiatExchangeRate: e.target.value })}
                              className="flex-1 w-full min-w-0 p-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-bold text-slate-900 dark:text-white outline-none transition-colors focus:border-amber-500"
                            />
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="relative mt-1">
                            <select
                              value={formData.destinationVault}
                              onChange={(e) => setFormData({ ...formData, destinationVault: e.target.value, destinationVaultName: '' })}
                              className="w-full p-4 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-700/50 rounded-xl font-bold text-slate-900 dark:text-white text-sm shadow-sm cursor-pointer appearance-none outline-none transition-colors focus:border-amber-500"
                            >
                        ...
                        {(formData.destinationVault === 'bankWallet' || formData.destinationVault === 'onlineWallet') && (
                          <div className="animate-in fade-in mt-1 sm:mt-0">
                            ...
                            <input
                              type="text" list="bridge-vaults" required
                              value={formData.destinationVaultName}
                              onChange={(e) => setFormData({ ...formData, destinationVaultName: e.target.value })}
                              placeholder="e.g., SBI or PayPal"
                              className="w-full mt-1 p-4 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-700/50 rounded-xl font-bold text-slate-900 dark:text-white text-sm shadow-sm placeholder-slate-400 outline-none transition-colors focus:border-amber-500"
                            />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={formData.fiatFee}
                          onChange={(e) => setFormData({ ...formData, fiatFee: e.target.value })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="datetime-local" required disabled={formData.isSynced}
                    value={formData.date}
                    onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                    className="w-full mt-1 p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none shadow-sm transition-colors cursor-pointer disabled:opacity-50 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full mt-1 p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none shadow-sm placeholder-slate-400 transition-colors focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50"
                  />
                </div>
              </div>
              ...
              <div className="sticky bottom-0 pt-2 pb-1 bg-white dark:bg-slate-900 mt-2 z-10">
                <button
                  ...
                  className={`w-full p-4 rounded-2xl font-black text-white text-sm uppercase tracking-widest shadow-xl transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 shrink-0 ${transactionType === 'in'
            ...
            <form onSubmit={executeSecureDelete} className="space-y-4 overflow-y-auto custom-scrollbar flex-1 pb-4">
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-500/30 rounded-xl shadow-sm">
                <p className="text-xs font-bold text-amber-800 dark:text-amber-300">
                  Deleting this will permanently remove <span className="font-black text-amber-900 dark:text-amber-200">{deleteContext.quantity} {deleteContext.coin}</span> from your records. Linked Fiat Bridge records (if any) will also be reversed automatically.
                </p>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="••••••"
                  className="w-full text-center tracking-[0.5em] text-2xl p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl font-black text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 shadow-sm transition-colors focus:border-rose-500"
                />
...
export default CryptoWalletContent;