...
const CryptoWallet = () => {
  ...
  const fetchMarketData = useCallback(async () => {
    ...
    if (coinsToFetch.length > 0) {
      ...
      await Promise.all(coinsToFetch.map(async (sym) => {
        ...
        if (isContractMode && dbCoin.contractAddress) {
          try {
  ...
  const handleSaveEntry = async (e) => {
    ...
    if (!formData.platform.trim()) return alert("Please enter a wallet/platform name!");
    
    if (transactionType === 'out') {
      if (qty > (holdings[formData.coin]?.platforms[formData.platform] || 0)) {
        return alert(`Insufficient Crypto Funds! Available: ${holdings[formData.coin]?.platforms[formData.platform] || 0}`);
      }
      if (isBridging && !editingId && (formData.destinationVault === 'bankWallet' || formData.destinationVault === 'onlineWallet') && !formData.destinationVaultName.trim()) {
        return alert("Please specify the exact Bank or Wallet Name for the Fiat conversion.");
      }
    }
  ...
  const openModal = (type) => { 
    ...
    setFormData(prev => ({ 
      ...
      destinationVaultName: existingVaultNames[0] || '', date: getLocalISOString() 
    })); 
    setIsModalOpen(true); 
  };
  ...
  const handleDownloadReport = (format) => {
    ...
    if (format === 'pdf') downloadPDFReport(reportData, columns, 'Crypto_Vault_Ledger', 'Crypto Vault Ledger');
    else downloadExcelReport(reportData, columns, 'Crypto_Vault_Ledger', 'Crypto Vault Ledger');
  };
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden max-h-[calc(100dvh-4rem)] sm:max-h-[85vh] border border-slate-300 dark:border-slate-700 animate-in slide-in-from-bottom-10 sm:zoom-in-95">
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1 space-y-6">
              ...
              <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <button type="button" disabled={editingId} onClick={() => { setTransactionType('in'); setIsBridging(false); }} className={`flex-1 py-3 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all ${transactionType === 'in' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30'}`}>Deposit / Buy</button>
                <button type="button" disabled={editingId} onClick={() => setTransactionType('out')} className={`flex-1 py-3 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all ${transactionType === 'out' ? 'bg-rose-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30'}`}>Withdraw / Sell</button>
              </div>
