...
const AiStrategy = () => {
  ...
  // ─── 🚀 ENTERPRISE SYNC ENGINE (Reads Summaries directly) ───
  const summaryQuery = useMemo(() => user ? query(collection(db, 'users', user.uid, 'walletSummary')) : null, [user]);
  const { data: summaryDocs, loading: summariesLoading } = useSecureSnapshot(summaryQuery);

  useEffect(() => {
    if (!user || summariesLoading) return;

    let unsubs = [];

    // 1. BANK & CASH (Raw fallback since they don't use the summary engine yet)
    unsubs.push(onSnapshot(collection(db, "users", user.uid, "bankWallet"), snap => {
      setBankBalance(calcVaultBalance(snap.docs.map(doc => doc.data())).balance);
    }));
    unsubs.push(onSnapshot(collection(db, "users", user.uid, "cashWallet"), snap => {
      setCashBalance(calcVaultBalance(snap.docs.map(doc => doc.data())).balance);
    }));

    // 2. ONLINE WALLET
    const onlineSummary = summaryDocs?.find(s => s.id === 'online');
    if (onlineSummary) {
      setOnlineBalance(onlineSummary.totalBalance || 0);
    } else {
      unsubs.push(onSnapshot(collection(db, "users", user.uid, "onlineWallet"), snap => {
        setOnlineBalance(calcVaultBalance(snap.docs.map(doc => doc.data())).balance);
      }));
    }
    ...
    if (cryptoSummary) {
      ...
      Object.entries(cryptoSummary.holdings || {}).forEach(([coin, data]) => {
        ...
        if (priceUSD > 0) bal += (data.total || 0) * priceUSD * fiatRate;
      });
      setCryptoBalance(bal);
    } else {
      unsubs.push(onSnapshot(collection(db, "users", user.uid, "cryptoWalletLogs"), snap => {
        const holdings = {};
        ...
        for (const [coin, qty] of Object.entries(holdings)) {
          ...
          if (priceUSD > 0) bal += qty * priceUSD * fiatRate;
        }
        setCryptoBalance(bal);
      }));
    }
    ...
    // 4. STAKING & YIELD
    const stakingSummary = summaryDocs?.find(s => s.id === 'staking');
    if (stakingSummary) {
      let bal = 0;
      (stakingSummary.activeStakes || []).forEach(s => {
        const p1 = livePrices[s.coin?.toUpperCase()]?.priceUSD || 0;
        bal += (s.principalAmount || 0) * p1 * fiatRate;
        if (s.earningType === 'pool') {
          const p2 = livePrices[s.poolCoin2?.toUpperCase()]?.priceUSD || 0;
          bal += (s.poolPrincipal2 || 0) * p2 * fiatRate;
        }
      });
      setStakedFiatValue(bal);
    } else {
      unsubs.push(onSnapshot(collection(db, "users", user.uid, "stakingLogs"), snap => {
        let bal = 0;
        snap.docs.forEach(doc => {
          const d = doc.data();
          const p1 = livePrices[d.coin?.toUpperCase()]?.priceUSD || 0;
          bal += (parseFloat(d.principalAmount) || 0) * p1 * fiatRate;
          if (d.earningType === 'pool') {
            const p2 = livePrices[d.poolCoin2?.toUpperCase()]?.priceUSD || 0;
            bal += (parseFloat(d.poolPrincipal2) || 0) * p2 * fiatRate;
          }
        });
        setStakedFiatValue(bal);
      }));
    ...
    const timeout = setTimeout(() => setDataLoaded(true), 800);

    return () => {
      unsubs.forEach(unsub => unsub());
      clearTimeout(timeout);
    };
  }, [user, summariesLoading, summaryDocs, livePrices, fiatRate]);
