...
// 🚀 FIXED: Added updateDoc for marking notifications as read
...
import { 
  FaRobot, FaBellSlash, FaBrain, FaMicrochip, FaCircle 
} from 'react-icons/fa';
...
const VoiceButton = ({ isSpeaking, isMuted, onToggle, isActive }) => (
  <button 
    onClick={onToggle} 
    ...
    <div className="relative z-10 flex flex-col flex-1">
      ...
      <div className="flex items-center gap-3 mt-auto pt-4 border-t border-slate-100 dark:border-slate-800/50">
        ...
        <button 
          onClick={(e) => { 
            e.stopPropagation(); 
            if(item.isFirebase) onDismiss(item.id); // Mark as read when action taken
...
const EmptyState = () => (
  <div className="text-center py-16 sm:py-24 px-4 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm col-span-full">
    ...
    <h3 className="text-xl sm:text-2xl font-black text-slate-700 dark:text-white mb-2">All Clear!</h3>
...
const SmartSuggestions = () => {
  ...
  const [localHiddenAlerts, setLocalHiddenAlerts] = useState([]); // For dismissing non-firebase alerts
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const notifs = snapshot.docs.map(doc => {
        ...
        // Determine icon type
        let iconType = 'bulb';
        if (data.type === 'goal_milestone') iconType = 'trendUp';
        else if (data.type === 'market_alert') {
          iconType = data.title?.toLowerCase().includes('dump') ? 'trendDown' : 'trendUp';
        } else if (data.type?.includes('alert')) iconType = 'shield';
        else if (data.type?.includes('system')) iconType = 'chip';
        
        // Determine type label
        let typeLabel = 'INFO';
        if (data.type === 'market_alert') typeLabel = 'MARKET ALERT';
        else if (data.type === 'goal_milestone') typeLabel = 'SUGGESTION';
        else if (data.type?.includes('alert')) typeLabel = 'ALERT';
        else if (data.type?.includes('system')) typeLabel = 'SYSTEM ALERT';
        
        return {
          id: doc.id,
          isFirebase: true, // Identify as DB record
          type: typeLabel,
          title: data.title || 'Notification',
          message: data.message || '',
          iconType,
          actionText: data.type === 'market_alert' ? 'View Market' : 
                      data.type === 'goal_milestone' ? 'View Goals' : 'View Details',
          actionLink: data.link || '/dashboard',
          rawTimestamp: data.timestamp // Kept raw for formatting
        };
      });
  ...
  const combinedAlerts = useMemo(() => {
    const merged = [...(liveAlerts || []).filter(la => !localHiddenAlerts.includes(la.id))];
    firebaseAlerts.forEach(fbAlert => {
      const exists = merged.some(live => live.title === fbAlert.title);
      if (!exists) merged.push(fbAlert);
    });
    return merged.sort((a, b) => (b.rawTimestamp || 0) - (a.rawTimestamp || 0)); // Sort newest first
  }, [liveAlerts, firebaseAlerts, localHiddenAlerts]);

  // 🚀 Mark Notification as Read (Dismiss)
  const handleDismiss = async (id) => {
    const alert = combinedAlerts.find(a => a.id === id);
    if (!alert) return;
    
    if (activeId === id) stop(); // Stop speaking if dismissing active alert

    if (alert.isFirebase && user) {
      try {
        await updateDoc(doc(db, "users", user.uid, "notifications", id), { isRead: true });
      } catch(e) { console.error("Failed to dismiss"); }
    } else {
      setLocalHiddenAlerts(prev => [...prev, id]); // Hide locally if it's a live AI alert
    }
  };

  // Track active speaking
  useEffect(() => {
          ...
          <>
            <div className="flex items-center justify-between px-2">
              <h2 className="text-sm font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                ...
                {combinedAlerts.length} Insight{combinedAlerts.length !== 1 ? 's' : ''}
              </h2>
