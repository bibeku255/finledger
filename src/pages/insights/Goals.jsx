...
const GoalsContent = () => {
  ...
  // 🚀 O(1) Vault Fetching Upgrade: Uses walletSummary instead of massive getDocs queries
  useEffect(() => {
    if (!user) return;
    const fetchVaults = async () => {
      try {
        ...
        // Fallback safety net if summary isn't built yet
        if (names.size === 0) {
        ...
        console.error("Failed to fetch vault names:", error);
      }
    };
    fetchVaults();
  }, [user]);
  ...
  const executeSecureDelete = async (e) => {
    ...
    try {
      ...
      if (currentSavedAmount > 0) {
        ...
        // ✅ Secure Schema: Added bankName explicitly
        await addDoc(collection(db, "users", user.uid, "bankWallet"), {
          title: `Goal Refund: ${deleteContext.title}`, type: 'in', date: formattedDate, timestamp, currency: baseCurrency, foreignAmount: currentSavedAmount, exchangeRate: 1, finalBaseAmount: currentSavedAmount, fee: 0, isGoalLock: true, linkedIncomeId: refundId, walletName: 'Auto Refund', bankName: 'Auto Refund', transferType: 'Refund/Reversal'
        });
