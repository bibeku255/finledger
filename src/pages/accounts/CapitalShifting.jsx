...
import {
  ...
  getDoc, query, where, getDocs, documentId
...
// 🚀 ENTERPRISE ENGINE: Syncs Shift Summary
const syncCapitalShiftsSummary = async (userId) => {
  try {
    const snap = await getDocs(collection(db, 'users', userId, 'capitalShifts'));
    const logs = snap.docs.map(d => d.data());

    let totalVolume = 0;
    const connectedVaults = new Set();

    logs.forEach(s => {
      totalVolume += (Number(s.grossAmount) || 0) * (Number(s.fromExchangeRate) || 1);
      if (s.fromVault) connectedVaults.add(s.fromVault);
      if (s.toVault) connectedVaults.add(s.toVault);
    });

    const summaryData = {
      totalShifts: logs.length,
      totalVolume,
      connectedVaultsCount: connectedVaults.size,
      lastUpdated: new Date().toISOString()
    };

    await setDoc(doc(db, 'users', userId, 'walletSummary', 'capitalShifts'), summaryData, { merge: true });
    return summaryData;
  } catch (error) {
    console.error("Failed to sync capital shifts summary:", error);
    return null;
  }
};
...
const CapitalShiftingContent = () => {
  ...
  const [globalSummary, setGlobalSummary] = useState({
    totalShifts: 0, totalVolume: 0, connectedVaultsCount: 0
  });

  const summaryQuery = useMemo(() => {
    if (!user?.uid) return null;
    return query(collection(db, 'users', user.uid, 'walletSummary'), where(documentId(), '==', 'capitalShifts'));
  }, [user?.uid]);
  ...
  useEffect(() => {
    ...
    if (summaryDataArray && summaryDataArray.length > 0) {
      ...
      syncCapitalShiftsSummary(user.uid).then(data => {
  ...
  } = usePaginatedMonthlyLogs(user?.uid, 'capitalShifts', 20, !!user, getCalendarMonthKey);
  ...
  // Fetch custom coins & OPTIMIZED existing vault names
  useEffect(() => {
    if (!user) return;
  ...
  // ✅ SUPER OPTIMIZED: Fetches Vault Names from Summary Docs instead of raw logs!
  useEffect(() => {
    if (!user) return;
    const fetchVaults = async () => {
      try {
        ...
        setExistingVaultNames(Array.from(names));
         ...
         console.warn("Could not load vault names from summary");
      }
    };
    fetchVaults();
  }, [user]);
  ...
  const filteredLedger = useMemo(() => {
    ...
    return paginatedMonths.map(month => {
      ...
      let pageGross = 0, pageFees = 0, pageNet = 0;
      month.records.forEach(shift => {
  ...
  }, [paginatedMonths, formatGlobalDate]);
  ...
  const handleExportPDF = async () => {
    ...
    if(recordsToExport.length === 0) { addToast('No records found.', 'warning'); setIsGeneratingPDF(false); return; }

    const reportData = recordsToExport.map(shift => {
      ...
      return {
        date: formatGlobalDate ? formatGlobalDate(shift.date, 'full') : shift.date?.split('T')[0],
        ...
        grossBase: Number(grossBase.toFixed(2)), feeBase: Number(feeBase.toFixed(2)), netBase: Number(netBase.toFixed(2)),
      };
    });

    const columns = [
      { header: 'Date', key: 'date' }, { header: 'Sent From', key: 'sourceStr' }, { header: 'Received In', key: 'destStr' },
      { header: 'Routing', key: 'routing' }, { header: `Gross (${currencySymbol})`, key: 'grossBase', isNumeric: true },
      { header: `Fees (${currencySymbol})`, key: 'feeBase', isNumeric: true }, { header: `Net Value (${currencySymbol})`, key: 'netBase', isNumeric: true },
    ];
    try {
      await downloadPDFReport(reportData, columns, 'Capital_Shifting_Ledger', 'Capital Shifting Audit', { onSuccess: () => addToast('PDF downloaded!', 'success'), onError: msg => addToast(`Error: ${msg}`, 'error') });
  ...
  const handleExportExcel = async () => {
    ...
    const recordsToExport = filteredLedger.flatMap(m => m.records);
    if(recordsToExport.length === 0) { addToast('No records found.', 'warning'); setIsGeneratingExcel(false); return; }
    ...
    const reportData = recordsToExport.map(shift => {
      ...
      return {
        Date: formatGlobalDate ? formatGlobalDate(shift.date, 'full') : shift.date?.split('T')[0],
        Source: `${Number(shift.grossAmount).toLocaleString()} ${shift.fromAsset} (${shift.fromVault.toUpperCase()}: ${fromPlatform})`,
        Destination: `${Number(shift.netReceived).toLocaleString()} ${shift.toAsset} (${shift.toVault.toUpperCase()}: ${toPlatform})`,
        Routing: shift.routingPlatform || 'Direct',
        Gross: grossBase.toFixed(2), Fee: feeBase.toFixed(2), NetValue: netBase.toFixed(2),
      };
    });
    ...
    const a = document.createElement('a'); a.href = url; a.download = `Capital_Shifting_Ledger.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  ...
  const initiateEditShift = (shift) => {
    setTransferData({
      fromVault: shift.fromVault, fromSubWallet: shift.fromSubWallet || '', fromCryptoPlatform: shift.fromCryptoPlatform || cryptoPlatformsList[0],
      fromAsset: shift.fromAsset, grossAmount: shift.grossAmount || '', fromExchangeRate: shift.fromExchangeRate || 1,
      networkFeeAsset: shift.networkFeeAsset || shift.fromAsset, networkFee: shift.networkFee || '', networkFeeExchangeRate: shift.networkFeeExchangeRate || 1,
      routingPlatform: shift.routingPlatform || '', routingAgent: shift.routingAgent || '',
      toVault: shift.toVault, toSubWallet: shift.toSubWallet || '', toCryptoPlatform: shift.toCryptoPlatform || cryptoPlatformsList[0],
      toAsset: shift.toAsset, netReceived: shift.netReceived || '', toExchangeRate: shift.toExchangeRate || 1,
      taxAndFees: shift.taxAndFees || '', date: shift.date, referenceId: shift.referenceId || '',
    });
  ...
  const executeShift = async () => {
    ...
    try {
      ...
      // ✅ SMART CROSS-VAULT SYNC
      // Delete summary documents of affected vaults so they rebuild next time they are visited!
      const vaultsToClear = [transferData.fromVault, transferData.toVault].filter(v => ['bank', 'cash', 'online'].includes(v));
      for (const v of new Set(vaultsToClear)) {
        await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', v));
      }
      
      await syncCapitalShiftsSummary(user.uid);
      refreshTransactions();
  ...
  const executeSecureDeleteShift = async e => {
    ...
    try {
      ...
      // ✅ SMART CROSS-VAULT SYNC
      const vaultsToClear = [deleteContext.fromVault, deleteContext.toVault].filter(v => ['bank', 'cash', 'online'].includes(v));
      for (const v of new Set(vaultsToClear)) {
        await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', v));
      }
      ...
      <div className="pt-20 sm:pt-24 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto px-4 md:px-6">
        ...
        <div className="relative rounded-[2.5rem] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 md:p-8 shadow-2xl border border-slate-700/50 z-20">
          ...
          <div className="relative z-30 grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mt-8 pt-6 border-t border-white/10">
            <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/5"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1.5"><HiOutlineSwitchHorizontal size={14}/> Total Shifts</p><p className="text-xl sm:text-2xl font-black text-white">{globalSummary.totalShifts || 0}</p></div>
            <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/5"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1.5"><HiOutlineTrendingUp size={14}/> Total Volume</p><p className="text-xl sm:text-2xl font-black text-white truncate" title={`${currencySymbol}${globalSummary.totalVolume}`}>{currencySymbol}{(globalSummary.totalVolume || 0).toLocaleString(undefined, {maximumFractionDigits: 0})}</p></div>
            <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/5 col-span-2 md:col-span-1"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1.5"><HiOutlineShieldCheck size={14}/> Connected Vaults</p><p className="text-xl sm:text-2xl font-black text-white">{globalSummary.connectedVaultsCount || 0}</p></div>
          </div>
        </div>
        ...
        {/* 🚀 TRUE INFINITE SCROLL HISTORY SECTION */}
        <div className="mt-10">
          ...
          {isLoadingLogs ? <HistorySkeleton /> : filteredLedger.length === 0 ? (
            ...
            <>
                    ...
                    <button onClick={() => toggleMonth(month.monthKey)}
                      ...
                      <div className="flex items-center gap-4">
                        <div className="hidden sm:flex gap-4 text-xs font-bold">
                          <span className="text-emerald-600">Vol: {currencySymbol}{month.pageGross.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
                          <span className="text-rose-600">Fees: {currencySymbol}{month.pageFees.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
                        </div>
                    ...
                    {isOpen && (
                      <div className="animate-in fade-in duration-200">
                        <div className="px-6 py-4 grid grid-cols-2 sm:grid-cols-4 gap-4 bg-white dark:bg-slate-900 border-t border-b border-slate-200 dark:border-slate-800">
                          <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Shifts Shown</p><p className="font-black text-slate-700 dark:text-slate-300">{month.records.length}</p></div>
                          <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Page Gross</p><p className="font-black text-indigo-600">{currencySymbol}{month.pageGross.toLocaleString(undefined, {minimumFractionDigits: 2})}</p></div>
                          <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Page Fees</p><p className="font-black text-rose-600">-{currencySymbol}{month.pageFees.toLocaleString(undefined, {minimumFractionDigits: 2})}</p></div>
                          <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Page Net</p><p className="font-black text-emerald-600">+{currencySymbol}{month.pageNet.toLocaleString(undefined, {minimumFractionDigits: 2})}</p></div>
                        </div>
                        <div className="flex flex-col divide-y divide-slate-100 dark:divide-slate-800/50">
                          {displayedRecords.map(shift => {
                                ...
                                <div className="flex items-end justify-between mt-1 pt-2">
                                  <div className="flex flex-col gap-1.5">
                                    <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800/50 px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 w-fit">
                                      <HiOutlineCalendar size={12} className="text-slate-500 shrink-0" /> {formatGlobalDate ? formatGlobalDate(shift.date, 'short') : shift.date.split('T')[0]} <span className="opacity-50 ml-1">{timeStr}</span>
                                    </p>
                          ...
                          {hasMoreRecords && (
                            <button onClick={() => toggleExpandMonth(month.monthKey)}
                              className="w-full py-4 px-6 text-center text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 font-black text-sm uppercase tracking-widest border-t border-slate-100 dark:border-slate-800/50 transition-colors flex items-center justify-center gap-2">
                              <HiOutlineChevronDown size={16} />
                              {isExpanded ? 'Show Less' : `View ${hiddenCount} more in ${month.monthName}`}
                            </button>
                          )}
                        </div>
                      </div>
              ...
              {hasMore && (
                <div className="flex justify-center py-6">
                  <button onClick={loadMore} disabled={isPaginating}
                    className="px-8 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/20 flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50">
                    {isPaginating ? <HiOutlineRefresh className="animate-spin" size={18} /> : <><HiOutlineChevronDown size={18} /> Load Older Shifts</>}
                  </button>
                </div>
              )}
            </>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={executeShift} disabled={isProcessing} className="flex-1 py-3 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-bold text-sm hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-[600] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden border border-slate-300 dark:border-slate-700">
            ...
            <form onSubmit={executeSecureDeleteShift} className="p-6 space-y-5">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-700 dark:text-slate-400 uppercase tracking-widest ml-1 text-center block">Security PIN</label>
                <input type="password" maxLength={6} required autoFocus value={pinInput} onChange={(e) => setPinInput(e.target.value)}
                  className="w-full text-center tracking-[0.3em] text-xl p-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-xl font-black text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 transition-colors shadow-sm focus:border-rose-500 placeholder-slate-400" placeholder="••••" />
                {pinError && <p className="text-xs font-bold text-rose-700 dark:text-rose-400 mt-2 text-center">{pinError}</p>}
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setDeleteContext(null)} className="flex-1 p-4 rounded-xl font-black text-sm bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 transition-colors shadow-sm uppercase tracking-widest">Cancel</button>
...
export default CapitalShiftingContent;