...
const OnlineWallet = () => {
  ...
  // 🚀 FIXED: PRO REPORTING LOGIC MAPPING 
  const handleDownloadReport = (format) => {
    setIsExportMenuOpen(false);
    const filteredRecords = processedLedger.flatMap(month => month.records);
    if (filteredRecords.length === 0) return alert("No records found to download.");
    ...
    const reportData = filteredRecords.map(rec => {
      ...
      return {
        ...
        fee: Number(rec.feeAmount || 0),        // 🚀 Passing raw number for Report Engine Total calculation
        netImpact: Number(rec.netChange || 0),  // 🚀 Passing raw number for Report Engine Total calculation
        notes: cleanNote
      };
    });
    ...
    const columns = [
      ...
      { header: 'Platform / Wallet', key: 'platform' }, { header: 'Fiat Amount', key: 'fiatFlow' },
  ...
  const openModal = () => {
    ...
    setFormData({ title: '', walletName: lastWallet, referenceNo: '', isP2P: false, foreignAmount: '', currency: baseCurrency, exchangeRate: 1, fee: '', date: getLocalDateTimeString(), isSynced: false });
    setIsModalOpen(true);
  };
      ...
      <div className="pt-20 sm:pt-24 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search platforms or details..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-14 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-slate-900 dark:text-white outline-none focus:border-purple-500 transition-all placeholder-slate-400 dark:placeholder-slate-500"
            />
          </div>
          <div className="flex gap-2">
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)}
              className="w-full sm:w-auto px-5 py-3.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-700 dark:text-slate-300 outline-none focus:border-purple-500 cursor-pointer transition-all uppercase tracking-widest"
            >
        ...
        <div className="space-y-6 sm:space-y-8">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="relative">
                <div className="absolute inset-0 bg-purple-500 rounded-full blur-2xl opacity-20 animate-pulse scale-150" />
                <HiOutlineRefresh className="animate-spin text-5xl text-purple-500 relative" />
              </div>
              <p className="text-xs font-black text-slate-500 uppercase tracking-widest mt-6 animate-pulse">Syncing Cloud Wallets...</p>
            </div>
          ) : processedLedger.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm px-4 text-center">
              <div className="w-24 h-24 bg-slate-100 dark:bg-slate-800 rounded-[2rem] flex items-center justify-center mb-5">
                <HiOutlineGlobe className="text-5xl text-slate-300 dark:text-slate-600" />
              </div>
              ...
              {!searchTerm && filterType === 'all' && (
                <button onClick={openModal} className="mt-6 bg-purple-600 hover:bg-purple-700 text-white font-black px-6 py-3 rounded-xl shadow-lg shadow-purple-500/30 transition-all active:scale-95">
                  Log E-Deposit Now
                </button>
              )}
            </div>
              ...
              <div key={month.monthName} className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-[2rem] overflow-hidden shadow-sm">
                ...
                <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50/80 dark:bg-slate-800/50">
                  ...
                  <div className="text-right">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Opening Balance</p>
                          ...
                          <tr key={rec.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group ${rec.isP2P ? 'bg-amber-50/40 dark:bg-amber-900/10' : ''}`}>
                            ...
                            <td className="p-4 text-right align-top">
                              ...
                              {rec.currency !== baseCurrency && (
                                <p className="text-[9px] font-bold text-slate-500 dark:text-slate-400 mt-1 tracking-wider uppercase">
                                  Rate: {Number(rec.exchangeRate) || 1}
                                </p>
                              )}
                            </td>
                            ...
                            <td className="p-4 pr-6 align-top">
                              <div className="flex items-center justify-end gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button onClick={() => handleEdit(rec)} className="p-2.5 bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 rounded-xl transition-all shadow-sm border border-slate-300 dark:border-slate-700 active:scale-95">
                                  <HiOutlinePencil size={16} />
                                </button>
                                <button onClick={() => initiateDelete(rec)} className="p-2.5 bg-slate-100 dark:bg-slate-800 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-500/20 rounded-xl transition-all shadow-sm border border-slate-300 dark:border-slate-700 active:scale-95" title={rec.linkedExpenseId || rec.linkedIncomeId || rec.shiftId ? "Force Delete Auto-Synced Entry" : "Delete"}>
                ...
                <div className="px-6 py-5 border-t border-slate-200 dark:border-slate-800 flex justify-end bg-slate-50/80 dark:bg-slate-800/50">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden max-h-[90dvh] sm:max-h-[85vh] animate-in slide-in-from-bottom-10 sm:zoom-in-95 border border-slate-300 dark:border-slate-700">
            ...
            <div className="px-6 sm:px-8 py-5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white flex justify-between items-center shrink-0">
              <h3 className="text-xl font-black flex items-center gap-2">
                <FaWallet size={20} /> {editingId ? 'Edit E-Wallet Record' : 'Log E-Deposit'}
              </h3>
              <button type="button" onClick={closeModal} className="p-2 bg-white/20 hover:bg-white/30 rounded-full text-white transition-colors active:scale-90">
                <HiOutlineX size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 space-y-6 flex-1 overflow-y-auto custom-scrollbar pb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  ...
                  <input type="text" list="online-wallets" required value={formData.walletName} onChange={(e) => setFormData({...formData, walletName: e.target.value})} placeholder="e.g., PayPal, Skrill" 
                    className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-purple-500/50 transition-colors placeholder-slate-400 shadow-sm" />
                ...
                <div className="space-y-1.5">
                  ...
                  <input disabled={formData.isSynced} type="text" required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} placeholder="e.g., Freelance Client" 
                    className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-purple-500/50 transition-colors placeholder-slate-400 disabled:opacity-60 shadow-sm" />
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <select value={formData.currency} onChange={(e) => setFormData({...formData, currency: e.target.value, exchangeRate: e.target.value === baseCurrency ? 1 : ''})} 
                      className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-purple-500/50 transition-colors cursor-pointer disabled:opacity-60 shadow-sm">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="number" step="any" required value={formData.foreignAmount} onChange={(e) => setFormData({...formData, foreignAmount: e.target.value})} placeholder="0.00" 
                      className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-lg text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-purple-500/50 transition-colors placeholder-slate-400 shadow-sm" />
                  </div>
                </div>
              )}
              ...
              {!formData.isSynced && formData.currency !== baseCurrency && (
                <div className="p-4 sm:p-5 bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800/50 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm animate-in fade-in">
                  <div className="flex items-center justify-between w-full sm:w-auto">
                    ...
                    <button type="button" onClick={fetchLiveRate} disabled={isFetchingRate} className="text-[10px] sm:hidden font-black bg-indigo-600 text-white px-3 py-1.5 rounded-lg flex items-center gap-1 disabled:opacity-50 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 flex-1 w-full">
                    ...
                    <input type="number" step="any" required value={formData.exchangeRate} onChange={(e) => setFormData({...formData, exchangeRate: e.target.value})} placeholder="Rate" 
                      className="flex-1 w-full min-w-0 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors disabled:opacity-60 text-center shadow-sm" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
                      Gateway Fee 
                      <span className="text-[8px] bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400 px-1.5 py-0.5 rounded uppercase">IN {formData.currency}</span>
                    </label>
                    <input type="number" step="any" value={formData.fee} onChange={(e) => setFormData({...formData, fee: e.target.value})} placeholder="0.00" 
                      className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 transition-colors disabled:opacity-60 placeholder-slate-400 shadow-sm" />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input type="text" value={formData.referenceNo} onChange={(e) => setFormData({...formData, referenceNo: e.target.value})} placeholder="e.g. TXN123..." 
                      className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors disabled:opacity-60 placeholder-slate-400 shadow-sm" />
                  </div>
                </div>
              )}
                ...
                <div className="p-4 sm:p-5 bg-slate-100 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-inner">
                  <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xl sm:text-2xl font-black tracking-tight text-indigo-600 dark:text-indigo-400">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex justify-between ml-1">
                  <span>Date & Time *</span>
                </label>
                <input disabled={formData.isSynced} type="datetime-local" required value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})}
                  className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors disabled:opacity-60 shadow-sm cursor-pointer" 
                />
              </div>
              ...
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-500/30 rounded-2xl shadow-sm">
                ...
                {(deleteContext.linkedIncomeId || deleteContext.linkedExpenseId || deleteContext.linkedPartyId) && (
                  <p className="text-[10px] text-rose-600 dark:text-rose-400 mt-2 font-bold flex items-center gap-1.5 bg-rose-50 dark:bg-rose-900/20 p-2 rounded-lg border border-rose-200 dark:border-rose-800/50">
                    <HiOutlineExclamationCircle size={14} className="shrink-0"/> Auto-synced entry. Deletion may cause ledger mismatch. Best practice is to delete from the source.
                  </p>
                )}
              </div>
