// src/pages/accounts/ExpenseTracker.jsx
import React, { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import { useAuth } from '../../hooks/useAuth';
import { collection, addDoc, doc, setDoc, deleteDoc, updateDoc, query, orderBy, where, getDocs, getDoc, documentId } from 'firebase/firestore';
...
// 🚀 ENTERPRISE ENGINE: Automatically syncs perfect math for Expense
const syncExpenseSummary = async (userId, getCalendarMonthKeyFallback) => {
  try {
    const snap = await getDocs(collection(db, 'users', userId, 'expenseLogs'));
    ...
    await setDoc(doc(db, 'users', userId, 'walletSummary', 'expense'), summaryData, { merge: true });
    return summaryData;
  } catch (error) {
    console.error("Failed to sync expense summary:", error);
    return null;
  }
};
...
const ExpenseTrackerContent = () => {
  ...
  const [globalSummary, setGlobalSummary] = useState({
    totalExpenseBase: 0, monthTotals: {}, totalEntries: 0
  });

  const summaryQuery = useMemo(() => {
    if (!user?.uid) return null;
    return query(collection(db, 'users', user.uid, 'walletSummary'), where(documentId(), '==', 'expense'));
  }, [user?.uid]);
  ...
  useEffect(() => {
    ...
    if (summaryDataArray && summaryDataArray.length > 0) {
      ...
      syncExpenseSummary(user.uid, getCalendarMonthKey).then(data => {
  ...
  } = usePaginatedMonthlyLogs(user?.uid, 'expenseLogs', 20, !!user, getCalendarMonthKey);
  ...
  // ✅ OPTIMIZED VAULT FETCHING from Summary & Parties
  useEffect(() => {
    if (!user) return;
    const fetchDynamicLists = async () => {
      try {
        ...
        console.warn("Dynamic lists fetch failed. Using fallback.", e);
      }
    };
    fetchDynamicLists();
  }, [user]);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      await downloadPDFReport(reportData, columns, 'Expense_Tracker_Ledger', 'Expense Ledger', {
  ...
  const handleExportExcel = async () => {
    ...
    const reportData = filteredForReport.map(rec => {
      ...
      return {
        Date: formatGlobalDate ? formatGlobalDate(rec.date, 'full') : (rec.date ? rec.date.split('T')[0] : 'N/A'),
        Payee: (rec.title || 'N/A').replace(/(\r\n|\n|\r)/gm, " "), 
        Category: rec.category || 'N/A', 
        Source: sourceText, 
        GrossAmount: `${(Number(rec.amount) || 0).toLocaleString()} ${rec.asset || baseCurrency}`, 
        Fee: Number(rec.feeAmount || 0).toFixed(2), 
        TotalValue: Number(rec.finalBaseAmount || 0).toFixed(2)
      };
    });
    ...
    const a = document.createElement('a'); a.href = url; a.download = `Expense_Tracker_Ledger.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  ...
  const createVaultRecord = (sourceData, linkId, amountToDeduct, feeToDeduct = 0) => {
    ...
    if (sourceData.vault === 'crypto') {
      return {
        ...
        data: {
          type: 'out', coin: sourceData.asset || 'USDT', quantity: grossNative, fee: feeNative, totalQuantity: grossNative + feeNative,
          platform: sourceData.cryptoPlatform || 'Unknown Platform', reason: `Expense: ${safeCat} (${safeTitle}) ${formData.isKhataSplit ? '[Shared]' : ''}`,
          referenceNo: linkId, date: safeDate, timestamp: new Date(safeDate).getTime() || Date.now(), linkedExpenseId: linkId
        }
      };
    }
    ...
    return {
      ...
      data: {
        ...
        type: 'out', feeType: 'inclusive', date: safeDate, timestamp: new Date(safeDate).getTime() || Date.now(),
        currency: sourceData.asset || baseCurrency, foreignAmount: grossNative, exchangeRate: rate,
        fee: feeBase, feeAsset: baseCurrency, finalBaseAmount: totalDeductionBase, isExpense: true,
        linkedExpenseId: linkId, walletName: sourceData.subWallet || 'Default Wallet', bankName: sourceData.subWallet || 'Default Bank',
        transferType: 'Payment/Expense', walletCategory: sourceData.vault === 'online' ? 'Fiat Wallet' : 'Fiat Wallet',
        vaultId: sourceData.vault === 'bank' ? 'bank_' + (sourceData.subWallet || 'Main').trim().toUpperCase() : sourceData.vault === 'online' ? 'online_' + (sourceData.subWallet || 'Main').trim().toUpperCase() : 'cash_main'
      }
    };
  };
  ...
  const handleSaveEntry = async (e) => {
    ...
    setIsSaving(true);
    const oldExp = editingId ? paginatedMonths.flatMap(m=>m.records).find(i => i.id === editingId) : null;
    const safeDate = formData.date || getLocalDateTimeString();
    ...
    try {
          ...
          if (formData.isKhataSplit) {
            ...
            for (let ks of formData.khataSplits) {
              ...
              await addDoc(collection(db, "users", user.uid, "parties", partyId, "ledger"), {
                type: 'give', amount: parseFloat(ks.amount) || 0, currency: formData.isSplit ? baseCurrency : (formData.asset || baseCurrency),
                exchangeRate: formData.isSplit ? 1 : (parseFloat(formData.exchangeRate) || 1), baseAmount: amountOwedBase, note: `Shared Bill: ${formData.title || 'Unknown'} (${formData.category || 'Other'})`,
      ...
      await syncExpenseSummary(user.uid, getCalendarMonthKey);
        ...
        <div className="relative rounded-[2.5rem] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 sm:p-8 lg:p-10 shadow-2xl border border-slate-700/50 z-20">
          ...
          <div className="relative z-30 grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mt-6">
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10 overflow-hidden">
              ...
              <p className="text-xl sm:text-2xl font-black text-white truncate">{currencySymbol}{(Number(globalSummary.totalExpenseBase) || 0).toLocaleString(undefined, {maximumFractionDigits: 0})}</p>
            </div>
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10 overflow-hidden">
              ...
              <p className="text-xl sm:text-2xl font-black text-rose-400 truncate">{currencySymbol}{(Number(globalSummary.monthTotals?.[getCurrentMonthKey()]) || 0).toLocaleString(undefined, {maximumFractionDigits: 0})}</p>
            </div>
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10 col-span-2 md:col-span-1 overflow-hidden">
              ...
              <p className="text-xl sm:text-2xl font-black text-white truncate">{globalSummary.totalEntries || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 bg-white dark:bg-slate-900 p-3 sm:p-4 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-slate-900 dark:text-white outline-none focus:border-rose-500 transition-all placeholder-slate-400 dark:placeholder-slate-500 shadow-sm"
            />
          </div>
          <div className="flex gap-2">
            <div className="relative w-full sm:w-auto">
              <select 
                ...
                className="w-full sm:w-auto pl-4 pr-10 py-3.5 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-black text-xs sm:text-sm text-slate-700 dark:text-slate-300 outline-none focus:border-rose-500 cursor-pointer transition-all shadow-sm appearance-none"
              >
          ...
          <>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right hidden sm:block">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Period Net Spent</p>
                        <p className="text-sm font-black text-rose-700 dark:text-rose-400">-{currencySymbol}{month.pageNet.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                      </div>
                  ...
                  {isOpen && (
                    <div className="animate-in fade-in duration-200">
                      <div className="px-6 py-4 grid grid-cols-2 sm:grid-cols-2 gap-4 bg-white dark:bg-slate-900 border-t border-b border-slate-200 dark:border-slate-800">
                        ...
                        <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Period Net Spent</p><p className="font-black text-rose-600">-{currencySymbol}{month.pageNet.toLocaleString(undefined, {minimumFractionDigits: 2})}</p></div>
                      </div>
                      ...
                      <div className="md:hidden flex flex-col divide-y divide-slate-100 dark:divide-slate-800/50">
                        ...
                        {hasMoreRecords && (<button onClick={() => toggleExpandMonth(month.monthKey)} className="w-full py-4 text-center text-rose-600 dark:text-rose-400 font-black text-sm uppercase tracking-widest border-t border-slate-100 dark:border-slate-800/50">{isExpanded ? 'Show Less' : `View ${hiddenCount} more in ${month.monthName}`}</button>)}
                      </div>
                      ...
                      <div className="hidden md:block overflow-x-auto custom-scrollbar">
                        ...
                        {hasMoreRecords && (<button onClick={() => toggleExpandMonth(month.monthKey)} className="w-full py-3 text-center text-rose-600 dark:text-rose-400 font-black text-sm uppercase tracking-widest">{isExpanded ? 'Show Less' : `View ${hiddenCount} more in ${month.monthName}`}</button>)}
                      </div>
                    </div>
                  )}
                </div>
            ...
            {hasMore && (
              <div className="flex justify-center py-6">
                <button onClick={loadMore} disabled={isPaginating} className="px-8 py-4 bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-rose-500/20 disabled:opacity-50 flex items-center gap-2 transition-all active:scale-95">
                  {isPaginating ? <HiOutlineRefresh className="animate-spin" size={18} /> : <><HiOutlineChevronDown size={18} /> Load Older Expenses</>}
                </button>
              </div>
            )}
          </>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 disabled:opacity-60 transition-colors shadow-sm placeholder-slate-400 dark:placeholder-slate-500"
                  />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="p-4 sm:p-6 bg-rose-50/50 dark:bg-slate-800/80 rounded-[1.5rem] sm:rounded-[2rem] border border-rose-200 dark:border-slate-700 shadow-sm space-y-4 sm:space-y-5 animate-in fade-in zoom-in-95">
                    ...
                    {(formData.vault === 'bank' || formData.vault === 'online') && (
                      <div className="space-y-1.5 animate-in fade-in">
                        ...
                        <input 
                          ...
                          className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 placeholder-slate-400 dark:placeholder-slate-500 shadow-sm transition-colors"
                    ...
                    {formData.vault === 'crypto' && (
                      <div className="space-y-1.5 animate-in fade-in">
                        ...
                        <input 
                          ...
                          className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 transition-colors shadow-sm placeholder-slate-400 dark:placeholder-slate-500"
                        />
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input 
                          ...
                          placeholder="0.00"
                          className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-lg text-rose-500 dark:text-rose-500 outline-none focus:ring-2 focus:ring-rose-500/50 disabled:opacity-60 shadow-sm placeholder-rose-300 dark:placeholder-slate-600 transition-colors"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  {formData.asset !== baseCurrency && (
                    <div className="p-4 sm:p-5 bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-800/50 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 shadow-sm animate-in fade-in">
                      <span className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2 shrink-0">
                        <FaExchangeAlt className="text-rose-500" /> Exchange Rate:
                      </span>
                      <div className="flex items-center gap-2 flex-1 w-full">
                        ...
                        <input 
                          ...
                          className="flex-1 w-full min-w-0 p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-black text-sm text-slate-900 dark:text-white outline-none disabled:opacity-60 shadow-sm focus:ring-2 focus:ring-rose-500/50 transition-colors text-center"
                        />
                      ...
                      <button 
                        ...
                        className="w-full sm:w-auto bg-rose-600 hover:bg-rose-700 text-white px-4 py-3 sm:py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest disabled:opacity-50 flex items-center justify-center gap-1.5 shadow-sm transition-colors shrink-0 active:scale-95"
                      >
                        ...
                        <div key={index} className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 w-full bg-white dark:bg-slate-800 p-2.5 sm:p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                          ...
                          <div className="w-full sm:w-36 shrink-0 flex items-center gap-2">
                            <input 
                              ...
                              className="w-full p-2.5 sm:p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg font-bold text-sm text-slate-900 dark:text-white outline-none placeholder-slate-400 dark:placeholder-slate-500 transition-colors focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 text-center" 
                            />
            ...
            <form onSubmit={executeSecureDelete} className="p-6 sm:p-8 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-500/30 rounded-2xl shadow-sm">
                <p className="text-xs font-bold text-amber-800 dark:text-amber-300 leading-relaxed">
                  ...
                  <span className="font-black"> {currencySymbol}{(Number(deleteContext.finalBaseAmount)||0).toLocaleString()}</span>
                </p>
      ...
      {isExportModalOpen && (
        <div className="fixed inset-0 z-[1000] bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95">
            <div className="px-6 py-5 bg-gradient-to-r from-rose-600 to-pink-600 text-white flex justify-between items-center">
...
export default ExpenseTrackerContent;