...
import { 
  ...
  HiOutlineLibrary // 🚀 FIXED: Added the missing import here!
} from 'react-icons/hi';
import { 
        ...
        <div className="animate-ticker-scroll flex gap-8 px-4">
          {[...tickerData, ...tickerData, ...tickerData].map((item, idx) => (
            <div key={idx} className="flex items-center gap-2.5 font-bold text-sm cursor-default group shrink-0">
              ...
              <span className="text-slate-900 dark:text-white font-black text-xs sm:text-sm">{currencySymbol}{Number(item.price).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 4})}</span>
...
const CurrencyBadge = ({ currency, amount }) => (
  <div className="bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 shrink-0 flex items-center gap-3 hover:bg-white dark:hover:bg-slate-800 transition-all duration-300 group shadow-sm min-w-[200px]">
    <div className="relative">
      ...
      <img src={`https://flagcdn.com/w40/${fiatFlagMap[currency] || 'un'}.png`} alt="" className="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-slate-700 shadow-lg group-hover:scale-105 transition-transform" />
    </div>
    <div className="min-w-0 flex-1">
      <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest truncate mb-0.5">Physical {currency}</p>
...
const CashWallet = () => {
  ...
  const [formData, setFormData] = useState({ 
    title: '', foreignAmount: '', currency: baseCurrency, exchangeRate: 1, date: getLocalDateTimeString() 
  });
  ...
  const currencyBalances = useMemo(() => {
    ...
    return Object.entries(balances).filter(([_, v]) => Math.abs(v) > 0.01).map(([curr, value]) => ({ currency: curr, value }));
  ...
  const processedLedger = useMemo(() => {
    ...
    return Object.keys(grouped).sort().reverse().map(key => {
      ...
      const filtered = monthData.records.filter(r => (r.title || '').toLowerCase().includes(searchTerm.toLowerCase()) && (filterType === 'all' || r.type === filterType)).reverse();
  ...
  const totalBalance = processedLedger.length > 0 && searchTerm === '' && filterType === 'all' 
    ...
    : transactions.reduce((acc, t) => acc + (t.type === 'in' ? Number(t.finalBaseAmount || t.amount || 0) : -Number(t.finalBaseAmount || t.amount || 0)), 0);

  const handleDownloadReport = (format) => {
    setIsExportMenuOpen(false);

    const filteredRecords = processedLedger.flatMap(month => month.records);
    if (filteredRecords.length === 0) return alert("No cash records found to download for current filters.");
  ...
  const handleSaveEntry = async (e) => { 
    ...
    try { 
      if (editingId) await setDoc(doc(db, "users", user.uid, "cashWallet", editingId), recordData, { merge: true }); 
      else await addDoc(collection(db, "users", user.uid, "cashWallet"), recordData); 
      closeModal(); 
    } catch (error) { 
  ...
  const executeSecureDelete = async (e) => { 
    ...
    try { 
      ...
      const hp = await hashPIN(pinInput.trim()); 
      const sp = userSnap.data()?.security?.pinHash || userSnap.data()?.securityPin || userSnap.data()?.pin; 
      if (sp && sp !== hp && sp !== pinInput.trim()) { 
  ...
  const openModal = () => { 
    setEditingId(null); 
    setFormData({ title: '', foreignAmount: '', currency: baseCurrency, exchangeRate: 1, date: getLocalDateTimeString() }); 
    setIsModalOpen(true); 
  };
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm font-medium text-slate-400 mt-1">Track your cash holdings & foreign currency</p>
              </div>
            </div>
        ...
        <div className="space-y-6 sm:space-y-8">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm px-4">
              ...
              <p className="text-lg font-black text-slate-700 dark:text-slate-300">No cash records found</p>
              <p className="text-sm font-medium text-slate-500 mt-2 text-center max-w-sm">
                {searchTerm || filterType !== 'all' ? 'Try adjusting your search or filters.' : 'Log your first physical cash entry to get started.'}
              </p>
        ...
        <div className="fixed inset-0 z-[500] bg-slate-950/80 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden max-h-[90dvh] sm:max-h-[85vh] animate-in slide-in-from-bottom-10 sm:zoom-in-95 border border-slate-300 dark:border-slate-700">
            <div className="px-6 sm:px-8 py-5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white flex justify-between items-center shrink-0">
              <h3 className="text-xl font-black flex items-center gap-2"><FaMoneyBillWave size={20}/> {editingId ? 'Edit Entry' : 'Add Physical Cash'}</h3>
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 space-y-5 overflow-y-auto custom-scrollbar flex-1 pb-2">
              <div className="space-y-1.5">
                ...
                <input type="text" required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} placeholder="e.g., Salary, Gift" className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all shadow-sm" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="number" step="any" required value={formData.foreignAmount} onChange={(e) => setFormData({...formData, foreignAmount: e.target.value})} placeholder="0.00" className="w-full p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-lg text-emerald-600 dark:text-emerald-400 outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all shadow-sm" />
                </div>
              </div>
              ...
              {formData.currency !== baseCurrency && (
                <div className="p-4 sm:p-5 bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm animate-in fade-in">
                  <div className="flex items-center justify-between w-full sm:w-auto">
                    ...
                    <button type="button" onClick={fetchLiveRate} disabled={isFetchingRate} className="text-[10px] sm:hidden font-black bg-emerald-600 text-white px-3 py-1.5 rounded-lg flex items-center gap-1 disabled:opacity-50"><HiOutlineRefresh className={isFetchingRate ? "animate-spin" : ""} size={12} /> Live Rate</button>
                  </div>
              ...
              <div className="sticky bottom-0 pt-3 pb-1 bg-white dark:bg-slate-900 mt-2 z-10">
                <button type="submit" disabled={isSaving} className={`w-full p-4 sm:p-5 rounded-2xl font-black text-sm sm:text-base uppercase tracking-widest text-white transition-all shadow-xl flex items-center justify-center gap-2 shrink-0 ${isSaving ? 'opacity-70 cursor-not-allowed' : 'active:scale-95'} bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 shadow-emerald-500/30`}>
                  {isSaving ? <HiOutlineRefresh className="animate-spin text-2xl" /> : <HiOutlineCash size={20} />}
                  {isSaving ? 'Processing...' : (editingId ? 'Update Ledger' : 'Add to Vault')}
                </button>
              </div>
            </form>
          </div>
