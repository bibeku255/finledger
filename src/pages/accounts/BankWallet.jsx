// src/pages/accounts/BankWallet.jsx
...
const BankWalletContent = () => {
  ...
  // 📌 Form – FIXED: 'date' property included
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q,
      ...
      error => {
        addToast('Failed to load bank transactions.', 'error');
        setIsLoading(false);
      }
    );
    return () => unsubscribe();
  ...
  // 🚀 Unified engine balance & fees
  const { balance: netBalance, totalFees } = useMemo(() => calcVaultBalance(transactions), [transactions]);

  // 🆕 Sub‑wallet balances (FIXED: Now fully supports OUT/Expense in Native Currency)
  const subWalletBalances = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      // CRITICAL FIX: Extract native/foreign amount directly!
      let nativeAmt = Number(t.foreignAmount !== undefined ? t.foreignAmount : (t.amount !== undefined ? t.amount : t.finalBaseAmount || 0));
      ...
      if (rawType === 'out' || rawType === 'expense') {
        ...
        // Convert base currency fee back to native currency
    ...
    return Object.values(balances).filter(b => Math.abs(b.value) > 0.01).sort((a, b) => b.value - a.value);
      ...
      if (rawType === 'out' || rawType === 'expense') {
        const feeType = (t.feeType || 'inclusive').toLowerCase();
        const deduct = feeType === 'exclusive' ? amt + Number(t.fee || 0) : amt;
        outs += deduct;
      } else {
        ins += amt;
      }
    });
    return { totalInflows: ins, totalOutflows: outs };
  }, [transactions]);


  // 🗓️ Processed ledger (FIXED: all date references corrected)
  ...
  // Export – FIXED: date properties in reportData and columns
  const handleDownloadReport = async format => {
    setIsExportMenuOpen(false);
    const filteredRecords = processedLedger.flatMap(month => month.records);
    if (filteredRecords.length === 0) { addToast('No bank records found.', 'warning'); return; }
    ...
    const reportData = filteredRecords.map(rec => {
      ...
      return {
        ...
        type: isOut ? 'Withdrawal/Shift (-)' : 'Deposit (+)',
    ...
    const reportTitle = `Bank Vault Ledger & Audit`;
  ...
  // ----------------------------------------------------
  // DEPOSIT CALCULATION (UI Form Preview)
  // ----------------------------------------------------
    ...
    try {
      ...
      if (editingId) {
        if (formData.isSynced) {
          // Synced record update – sirf bankName aur vaultId
          await setDoc(doc(db, 'users', user.uid, 'bankWallet', editingId), {
            bankName: formData.bankName.trim(),
            vaultId: 'bank_' + formData.bankName.trim().toUpperCase(),
          }, { merge: true });
        } else {
          // Normal non-synced record update (full update)
          const recordData = {
  ...
  const handleEdit = rec => {
    ...
    setFormData({
      ...
      exchangeRate: rec.exchangeRate || 1, fee: rec.fee || '',
  ...
  const openModal = () => {
    ...
    setFormData({
      title: '', bankName: lastBank, transferType: 'UPI', referenceNo: '', isP2P: false,
      foreignAmount: '', currency: baseCurrency, exchangeRate: 1, fee: '',
      date: getLocalDateTimeString(), isSynced: false,
    });
    setIsModalOpen(true);
    ...
    <div className="w-full h-auto pb-24">
      <div className="pt-20 sm:pt-24 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-blue-700 via-indigo-700 to-blue-800 p-6 sm:p-8 lg:p-10 shadow-2xl border border-blue-400/30">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white tracking-tight">Bank Vault</h1>
                <p className="text-xs sm:text-sm font-medium text-blue-100 mt-1">Manage institutional assets and deposits</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
              <div className="relative flex-1 lg:flex-none">
                <button onClick={() => !isGeneratingReport && setIsExportMenuOpen(!isExportMenuOpen)} onBlur={() => setTimeout(() => setIsExportMenuOpen(false), 200)} disabled={isGeneratingReport} className="w-full flex items-center justify-center gap-2 px-5 py-3.5 bg-white/10 hover:bg-white/20 text-white rounded-2xl font-black text-xs uppercase tracking-widest backdrop-blur-sm transition-all border border-white/10 active:scale-95 shadow-sm disabled:opacity-50">
              ...
              <button onClick={openModal} className="flex-1 lg:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-blue-500/30">
          ...
          <div className="relative z-10 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-8 pt-6 border-t border-white/20">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10">
              ...
              <p className="text-xl sm:text-2xl font-black text-rose-300 truncate">-{currencySymbol}{totalFees.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10">
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-1.5 mb-1"><HiOutlineLibrary size={14} /> Entries</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          <StatCard title="Total Deposits" value={`${currencySymbol}${totalInflows.toLocaleString(undefined, { minimumFractionDigits: 0 })}`} icon={FaArrowDown} color="from-emerald-500 to-teal-600" subtitle="Gross money added" />
          <StatCard title="Total Withdrawn" value={`${currencySymbol}${totalOutflows.toLocaleString(undefined, { minimumFractionDigits: 0 })}`} icon={FaArrowUp} color="from-rose-500 to-pink-600" subtitle="Includes Shifts & Expenses" />
        ...
        {subWalletBalances.length > 0 && (
          <div className="p-5 sm:p-6 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="flex gap-4 overflow-x-auto custom-scrollbar pb-3">
              {subWalletBalances.map((item, idx) => (<BankBadge key={idx} bank={item.bank} currency={item.currency} amount={item.value} />))}
            </div>
          </div>
        )}
        ...
        {isLoading ? <LedgerSkeleton /> : processedLedger.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm px-4 animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm font-medium text-slate-500 mt-2 text-center max-w-sm">{searchTerm || filterType !== 'all' ? 'Try adjusting your search or filters.' : 'Log your first bank deposit to get started.'}</p>
            {!searchTerm && filterType === 'all' && (<button onClick={openModal} className="mt-6 bg-blue-600 hover:bg-blue-700 text-white font-black px-6 py-3 rounded-xl shadow-lg shadow-blue-500/30 transition-all active:scale-95">Log Deposit Now</button>)}
          </div>
        ) : (
          processedLedger.map(month => {
            ...
            // Calculate monthly totals cleanly based on type
            const totalIn = month.records.filter(r => r.type !== 'out' && r.type !== 'expense').reduce((s, r) => s + (r.finalBaseAmount || 0), 0);
            const totalOut = month.records.filter(r => r.type === 'out' || r.type === 'expense').reduce((s, r) => {
                const feeType = (r.feeType || 'inclusive').toLowerCase();
                const amt = Number(r.finalBaseAmount || 0);
                const fee = Number(r.fee || 0);
                return s + (feeType === 'exclusive' ? amt + fee : amt);
            }, 0);
                ...
                <button onClick={() => toggleMonth(month.monthKey)} className="w-full px-6 py-5 flex justify-between items-center bg-slate-50/80 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800/70 transition-colors text-left">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-sm font-black text-blue-700 dark:text-blue-400">{currencySymbol}{closing.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                    </div>
                ...
                {isOpen && (
                  <div className="animate-in fade-in duration-200">
                    <div className="px-6 py-4 grid grid-cols-2 sm:grid-cols-4 gap-4 bg-white dark:bg-slate-900 border-t border-b border-slate-200 dark:border-slate-800">
                      ...
                      <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Deposits (+)</p><p className="font-black text-emerald-600">+{currencySymbol}{totalIn.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div>
                      <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Withdrawals (-)</p><p className="font-black text-rose-600">-{currencySymbol}{totalOut.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div>
                      <div><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Closing</p><p className="font-black text-blue-700 dark:text-blue-400">{currencySymbol}{closing.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div>
                    </div>

                    {/* MOBILE CARDS – FIXED: rec.date references */}
                          ...
                          <div key={rec.id} className={`p-4 sm:p-5 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors flex flex-col gap-3 group ${rec.isP2P ? 'bg-amber-50/30 dark:bg-amber-900/10' : ''}`}>
                            <div className="flex justify-between items-start gap-2 w-full">
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${isOut ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'}`}>
                                  {isOut ? <FaArrowUp size={16} /> : <FaArrowDown size={16} />}
                                </div>
                                <div className="flex flex-col min-w-0 flex-1">
                                  ...
                                  <p className="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 break-words flex flex-wrap items-center gap-x-1.5">
                                    <FaUniversity className="shrink-0" /><span className="text-slate-700 dark:text-slate-300 font-black">{rec.bankName || 'Main Bank'}</span><span className="opacity-50 shrink-0">•</span><span>{rec.transferType || 'Transfer'}</span>
                                  </p>
                                </div>
                              </div>
                              <div className="text-right shrink-0 ml-2">
                                <p className={`text-base sm:text-lg font-black ${isOut ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                                  {netChangeText}
                                </p>
                              ...
                              <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
                                <div className="min-w-0">
                                  ...
                                  <p className="font-black text-slate-800 dark:text-slate-200 text-xs sm:text-sm break-words">
                                    {rec.currency !== baseCurrency ? `${(rec.foreignAmount || 0).toLocaleString()} ${rec.currency}` : `${currencySymbol}${(rec.foreignAmount || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`}
                                  </p>
                                  {rec.currency !== baseCurrency && <p className="text-[8px] sm:text-[9px] font-bold text-slate-400 break-words">≈ {currencySymbol}{((rec.foreignAmount || 0) * (rec.exchangeRate || 1)).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>}
                                </div>
                                {rec.feeAmount > 0 && (
                            ...
                            <div className="flex items-end justify-between mt-1">
                              <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800/50 px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 w-fit shrink-0">
                                  <HiOutlineCalendar size={12} className="text-slate-500 shrink-0" /> <span className="whitespace-nowrap">{formatGlobalDate ? formatGlobalDate(rec.date, 'short') : rec.date.split('T')[0]}</span> <span className="opacity-50 ml-1 whitespace-nowrap">{timeStr}</span>
                                </p>
                    ...
                    {/* DESKTOP TABLE – FIXED: rec.date references */}
                    <div className="hidden md:block overflow-x-auto custom-scrollbar">
                      <table className="w-full text-left min-w-[800px]">
                        <thead className="bg-white dark:bg-slate-900 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-200 dark:border-slate-800">
                          <tr><th className="p-4 pl-6 w-20">Type</th><th className="p-4">Details</th><th className="p-4 text-right">Gross Amount</th><th className="p-4 text-right">Net Impact</th><th className="p-4 text-right">Fee</th><th className="p-4 pr-6 text-right">Actions</th></tr>
                        </thead>
                              ...
                              <tr key={rec.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group ${rec.isP2P ? 'bg-amber-50/30 dark:bg-amber-900/10' : ''}`}>
                                ...
                                <td className="p-4">
                                  <div className="flex items-start gap-3.5">
                                    ...
                                    <div className="min-w-0">
                                      <p className="font-black text-slate-800 dark:text-slate-100 text-sm flex items-center gap-2 truncate">{rec.title}{rec.isP2P && (<span className="px-1.5 py-0.5 rounded-md bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400 text-[8px] font-black uppercase tracking-widest flex items-center gap-1 border border-amber-200 dark:border-amber-500/30"><FaShieldAlt size={8}/> P2P</span>)}</p>
                                      <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mt-1 truncate"><span className="text-slate-700 dark:text-slate-300 font-black">{rec.bankName || 'Main Bank'}</span> • {rec.transferType || 'Transfer'}</p>
                                      <div className="flex items-center gap-2 mt-1.5">
                                        <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{formatGlobalDate ? formatGlobalDate(rec.date, 'short') : rec.date.split('T')[0]}<span className="opacity-50 mx-1">•</span>{timeStr}</p>
                                        {rec.referenceNo && <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded truncate max-w-[120px]">Ref: {rec.referenceNo}</span>}
                                      </div>
                                    </div>
                                  </div>
                                </td>
                                <td className="p-4 text-right align-top">
                                  ...
                                  {rec.currency !== baseCurrency && <p className="text-[10px] font-bold text-slate-500 mt-1">≈ {currencySymbol}{((rec.foreignAmount || 0) * (rec.exchangeRate || 1)).toLocaleString(undefined, {minimumFractionDigits: 2})}</p>}
                                </td>
                                <td className="p-4 text-right align-top">
                                ...
                                <td className="p-4 text-right align-top">
                                  {rec.feeAmount > 0 ? <span className="text-xs font-bold text-rose-500">-{currencySymbol}{rec.feeAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span> : <span className="text-xs text-slate-400">-</span>}
                                </td>
                                <td className="p-4 pr-6 align-top">
                                  <div className="flex items-center justify-end gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(rec)} className="p-2 bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 rounded-lg transition-all active:scale-95 shadow-sm border border-slate-200 dark:border-slate-700"><HiOutlinePencil size={14} /></button>
                                    <button onClick={() => initiateDelete(rec)} className="p-2 bg-slate-100 dark:bg-slate-800 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-500/20 rounded-lg transition-all active:scale-95 shadow-sm border border-slate-200 dark:border-slate-700"><HiOutlineTrash size={14} /></button>
                                  </div>
                                </td>
                              </tr>
                            );
                    ...
                    <div className="px-5 sm:px-6 py-4 sm:py-5 border-t border-slate-200 dark:border-slate-800 flex justify-end bg-slate-50/50 dark:bg-slate-800/30">
                      <div className="text-right bg-white dark:bg-slate-900 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm shrink-0">
                        ...
                        <p className="text-base sm:text-lg font-black text-blue-700 dark:text-blue-400 whitespace-nowrap">{currencySymbol}{closing.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                      </div>
                    </div>
                  </div>
                )}
      ...
      {/* ADD DEPOSIT MODAL – FIXED: formData.date references */}
      {isModalOpen && (
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-700 animate-in slide-in-from-bottom-10 sm:zoom-in-95">
            <div className="px-6 sm:px-8 py-4 sm:py-5 flex justify-between items-center shrink-0 bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
              <h3 className="text-lg sm:text-xl font-black flex items-center gap-2">
                <FaArrowDown size={20} />
                {editingId ? 'Edit Deposit' : 'Log Bank Deposit'}
              </h3>
            ...
            <form onSubmit={handleSaveEntry} className="p-6 sm:p-8 space-y-5 overflow-y-auto custom-scrollbar flex-1 pb-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Bank Fee (in {baseCurrency})</label>
              ...
              <div className="p-4 sm:p-5 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-inner mt-2">
                ...
                {feeDeduction > 0 && <div className="flex justify-between text-xs sm:text-sm font-bold text-rose-500 mt-2"><span>Deductions:</span><span className="font-black">-{currencySymbol}{feeDeduction.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>}
              ...
              <div className="sticky bottom-0 pt-3 pb-1 bg-white dark:bg-slate-900 z-10 border-t border-slate-100 dark:border-slate-800 mt-4 sm:mt-0 sm:border-none">
                <button type="submit" disabled={isSaving} className="w-full p-4 sm:p-5 rounded-2xl font-black text-sm uppercase tracking-widest text-white transition-all disabled:opacity-60 flex items-center justify-center gap-2 active:scale-95 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-500/30">
                  {isSaving ? <HiOutlineRefresh className="animate-spin" size={20} /> : <FaArrowDown size={16} />}
                  {isSaving ? 'Processing...' : editingId ? 'Update Deposit' : 'Log Deposit'}
                </button>
              </div>
            </form>
          </div>
