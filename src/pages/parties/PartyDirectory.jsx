...
import { collection, addDoc, doc, setDoc, deleteDoc, query, orderBy, getDoc, getDocs, where } from 'firebase/firestore';
import { db } from '../../firebase/firebaseConfig';
import { downloadExcelReport, downloadPDFReport } from '../../utils/reportUtils';
import { verifyPINEnhanced } from '../../utils/securityUtils';
import { useToast } from '../../hooks/useToastNotification';

// ✅ IMPORTING ENTERPRISE HOOK
...
const PartyDirectoryContent = () => {
  ...
  // 🚀 ENTERPRISE ENGINE: useSecureSnapshot for Directory
  const partiesQuery = useMemo(() => {
    if (!user?.uid) return null;
    return query(collection(db, "users", user.uid, "parties"), orderBy("createdAt", "desc"));
  }, [user?.uid]);

  const { data: partiesData, loading: isLoading } = useSecureSnapshot(partiesQuery);
  ...
  // 🚀 ZERO-COST VAULT FETCHING: Reads from `walletSummary` instead of downloading thousands of logs!
  useEffect(() => {
    const fetchUserDataAndVaults = async () => {
      ...
      try {
        ...
        console.warn("Summary fetch failed. Fast autocomplete might be empty.", e);
      }
    };
  ...
  // Fast math for UI (Since directory is small, array methods are perfectly safe)
      ...
      if (editingPartyId) {
        ...
        if (startingBalanceBase !== 0) {
          ...
          if (formData.vault !== 'none') {
            ...
            if (['bank', 'cash', 'online', 'crypto'].includes(formData.vault)) {
  ...
  const executeSecureDelete = async (e) => {
    ...
    try {
      ...
      const delPromises = snap.docs.map(d => deleteDoc(doc(db, "users", user.uid, "parties", deleteContext.id, "ledger", d.id)));
      ...
      for (const colName of collectionsToCheck) {
        ...
        const colDelPromises = vSnap.docs.map(d => deleteDoc(doc(db, "users", user.uid, colName, d.id)));
        await Promise.all(colDelPromises);
      }

      // ✅ SMART CROSS-VAULT SYNC (Nuke summaries so they rebuild accurately without the deleted party)
      const vaultsToClear = ['bank', 'cash', 'online', 'crypto', 'income', 'expense'];
  ...
  const handleExportPDF = async () => {
    ...
    if (filteredParties.length === 0) { addToast("No records found.", "warning"); setIsGeneratingPDF(false); return; }
    ...
    try { await downloadPDFReport(reportData, columns, 'Smart_Khata_Directory', 'Smart Khata & Loans Directory', { onSuccess: () => addToast('PDF downloaded!', 'success'), onError: msg => addToast(`Error: ${msg}`, 'error') }); } 
  ...
  const handleExportExcel = async () => {
    ...
    if (filteredParties.length === 0) { addToast("No records found.", "warning"); setIsGeneratingExcel(false); return; }

    const reportData = filteredParties.map(p => ({
      Name: p.name,
      Type: p.accountType === 'loan' ? 'Formal Loan' : 'Casual Khata',
      Phone: p.phone || 'N/A',
      NetBalance: Number(p.netBalance || 0).toFixed(2),
      Status: p.status === 'bad_debt' ? 'Bad Debt' : p.netBalance === 0 ? 'Settled' : 'Active'
    }));
    ...
    const a = document.createElement('a'); a.href = url; a.download = `Smart_Khata_Directory.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800/50 rounded-xl shadow-sm">
                  ...
                  <div>
                    ...
                    <input type="date" required value={formData.emiDueDate} onChange={(e) => setFormData({...formData, emiDueDate: e.target.value})}
                      className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-sm transition-colors cursor-pointer" />
                  </div>
                </div>
              )}
                    ...
                    <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-300 dark:border-slate-700 shadow-sm space-y-4">
                      <div className="flex flex-row gap-2 sm:gap-3 w-full">
                        <select value={formData.currency} onChange={(e) => setFormData({...formData, currency: e.target.value, exchangeRate: e.target.value === baseCurrency ? 1 : ''})}
                          className="w-[35%] sm:w-auto min-w-[80px] p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none shadow-sm cursor-pointer transition-colors focus:border-blue-500 appearance-none">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="relative">
                            <select value={formData.vault} onChange={(e) => setFormData({...formData, vault: e.target.value, subWallet: ''})}
                              className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-white outline-none cursor-pointer shadow-sm transition-colors focus:border-blue-500 appearance-none">
            ...
            <form onSubmit={executeSecureDelete} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-center tracking-[0.3em] text-2xl p-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-xl font-black text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-rose-500/50 transition-colors shadow-sm focus:border-rose-500"
                />
                {pinError && <p className="text-xs font-bold text-rose-700 dark:text-rose-400 mt-2 text-center">{pinError}</p>}
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setDeleteContext(null)} className="flex-1 p-4 rounded-xl font-black text-sm bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 transition-colors shadow-sm uppercase tracking-widest active:scale-95">
                  Cancel
                </button>
        ...
        <div className="fixed inset-0 z-[1000] bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95">
            <div className="px-6 py-5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white flex justify-between items-center">
              <h3 className="text-xl font-black flex items-center gap-2"><HiOutlineDownload size={24} /> Export Directory</h3>
...
export default PartyDirectoryContent;