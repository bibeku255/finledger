...
const PartyLedgerContent = () => {
  ...
  useEffect(() => {
    ...
    const fetchDynamicLists = async () => {
      try {
        ...
        console.warn("Summary fetch failed. Relying on defaults.", e);
      }
    };
    fetchDynamicLists();
  }, [user]);
  ...
  const executeSecureDelete = async (e) => {
    ...
    try {
      ...
      // 🚀 AUTO INVALIDATE VAULT SUMMARIES
      if (entry.vault && ['bank', 'cash', 'online', 'crypto'].includes(entry.vault)) {
         await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', entry.vault));
         await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', 'expense'));
         await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', 'income'));
      }
  ...
  // ✅ MOBILE FRIENDLY EXPORT MODAL
  const handleExportPDF = async () => {
    setIsExportModalOpen(false);
    if (ledger.length === 0) { addToast("No transactions to download.", "warning"); return; }
    ...
    const reportData = ledger.map(entry => {
      ...
      let creditValue = 0, debitValue = 0;
      
      if (entry.type === 'receive' || (entry.type === 'opening_balance' && entry.baseAmount < 0)) creditValue = Math.abs(entry.baseAmount);
      else if (entry.type === 'give' || entry.type === 'emi_payment' || entry.type === 'interest' || (entry.type === 'opening_balance' && entry.baseAmount > 0)) debitValue = Math.abs(entry.baseAmount);

      return {
        date: formatGlobalDate ? formatGlobalDate(entry.date, 'full') : (entry.date ? entry.date.split('T')[0] : 'N/A'),
        action: action,
        nativeAmount: `${(Number(entry.amount) || 0).toLocaleString()} ${entry.currency || baseCurrency}`,
        credit: creditValue, debit: debitValue, note: cleanNote
      };
    });

    const columns = [
      { header: 'Date', key: 'date' }, { header: 'Action', key: 'action' }, { header: 'Native Amount', key: 'nativeAmount' },
      { header: `Given/Paid (${currencySymbol})`, key: 'debit', isNumeric: true }, { header: `Received (${currencySymbol})`, key: 'credit', isNumeric: true },
      { header: 'Details / Notes', key: 'note' }
    ];
    
    try { await downloadPDFReport(reportData, columns, `Khata_${party.name.replace(/\s+/g, '_')}`, `${party.name.toUpperCase()} - Khata Ledger`, { onSuccess: () => addToast('PDF downloaded!', 'success'), onError: msg => addToast(`Error: ${msg}`, 'error') }); } 
    catch (e) { addToast('Failed to generate report.', 'error'); }
  };
  ...
  const handleExportExcel = async () => {
    ...
    const reportData = ledger.map(entry => {
      ...
      return {
        ...
        Action: action,
        NativeAmount: `${(Number(entry.amount) || 0).toLocaleString()} ${entry.currency || baseCurrency}`,
        Given: debitValue, Received: creditValue, Notes: cleanNote
      };
    });
    ...
    const a = document.createElement('a'); a.href = url; a.download = `Khata_${party.name.replace(/\s+/g, '_')}.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  ...
  const handleTransaction = async (e) => {
    ...
    try {
      ...
      // 🚀 AUTO INVALIDATE VAULT SUMMARIES
      if (formData.vault && ['bank', 'cash', 'online', 'crypto'].includes(formData.vault)) {
         await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', formData.vault));
         await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', 'expense'));
         await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', 'income'));
      }
    ...
    try {
      if (type === 'settle') {
        ...
        if (party.netBalance > 0) {
          ...
          // Note: Bad debt write-off artificially hits cash summary to show expense
          await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', 'cash'));
          await deleteDoc(doc(db, 'users', user.uid, 'walletSummary', 'expense'));
        }
        await addDoc(collection(db, "users", user.uid, "parties", party.id, "ledger"), {
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      ...
                      <input type="date" required value={formData.startDate} onChange={(e) => setFormData({...formData, startDate: e.target.value})}
                        className="w-full p-3 sm:p-4 bg-white dark:bg-slate-900 border border-blue-300 dark:border-blue-800 rounded-xl font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/50 shadow-sm text-sm sm:text-base cursor-pointer" />
                    </div>
                    <div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl overflow-hidden border border-slate-300 dark:border-slate-700 flex flex-col">
            ...
            <form onSubmit={executeSecureDelete} className="p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-5">
              ...
              <div className="flex gap-2 sm:gap-3 pt-2">
                <button type="button" onClick={() => setDeleteContext(null)} className="flex-1 p-3 sm:p-4 rounded-xl font-black text-xs sm:text-sm bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors border border-slate-300 dark:border-slate-700 shadow-sm active:scale-95 uppercase tracking-widest">
                  Cancel
                </button>
                <button type="submit" disabled={isVerifying || !pinInput} className="flex-1 p-3 sm:p-4 rounded-xl font-black text-white bg-rose-600 hover:bg-rose-700 transition-colors disabled:opacity-50 shadow-lg shadow-rose-500/30 active:scale-95 flex justify-center items-center gap-2 text-xs sm:text-sm uppercase tracking-widest">
        ...
        <div className="fixed inset-0 z-[1000] bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95">
            <div className="px-6 py-5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white flex justify-between items-center">
              <h3 className="text-xl font-black flex items-center gap-2"><HiOutlineDownload size={24} /> Export Statement</h3>
        ...
        <div className="fixed inset-0 z-[400] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl p-5 sm:p-6 md:p-8 text-center border border-slate-300 dark:border-slate-700">
            ...
            <div className="flex gap-2 sm:gap-3">
              <button onClick={closeModal} className="flex-1 py-3 sm:py-4 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 rounded-xl font-black text-xs sm:text-sm transition-colors hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 shadow-sm active:scale-95 uppercase tracking-widest">
                Cancel
              </button>
              <button onClick={() => handleSettlement(activeModal)} disabled={isProcessing} className={`flex-1 py-3 sm:py-4 text-white rounded-xl font-black text-xs sm:text-sm transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 uppercase tracking-widest ${
