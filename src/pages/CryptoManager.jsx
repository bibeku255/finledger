...
const fetchSolanaTokenMetadata = async (mintAddress, signal) => {
  ...
  try {
    ...
    // ---------- ISSUE #2 FIX: DexScreener Token Metadata with validation ----------
...
const fetchSolanaTokenPrice = async (mintAddress, signal) => {
  ...
  try {
    // ---------- ISSUE #3 FIX: DexScreener Price Fetch with validation ----------
    const dexResponse = await fetchWithRetry(`https://api.dexscreener.com/latest/dex/tokens/${mintAddress}`, { signal });
    if (dexResponse && dexResponse.ok) {
      const data = await dexResponse.json();
      // ✅ VALIDATE: response structure
      if (!data || typeof data !== 'object') {
        throw new Error('Invalid DexScreener price response');
      }
      if (Array.isArray(data.pairs) && data.pairs.length > 0) {
        const pair = data.pairs[0];
        const priceValue = parseFloat(pair?.priceUsd);
        // ✅ VALIDATE: price is finite number
        if (!Number.isFinite(priceValue) || priceValue <= 0) {
          throw new Error(`Invalid price: ${pair?.priceUsd}`);
        }
        setCache(cacheKey, priceValue, 5);
        return priceValue;
      }
    }
    return 0;
...
// ============================================
// 🚀 API FETCHING FUNCTIONS (with validation)
// ============================================
const fetchCoinGeckoCoins = async (page = 1, perPage = 250) => {
  try {
    ...
    if (res && res.ok) {
      ...
      // ---------- ISSUE #4 FIX ----------
      // ✅ VALIDATE: response must be array
      if (!Array.isArray(data)) {
...
const fetchCoinCapCoins = async (limit = 100) => {
  try {
    ...
    if (!res || !res.ok) {
      throw new Error(`CoinCap API error: ${res?.status || 'unknown'}`);
    }
    const data = await res.json();
    // ✅ VALIDATE: response structure
    if (!data || !data.data || !Array.isArray(data.data)) {
      throw new Error('CoinCap returned invalid data structure');
    }
    return data.data.map(coin => ({
...
const CryptoManagerContent = () => {
  ...
  const [isCoinDataLoading, setIsCoinDataLoading] = useState(true);
  ...
  // Live prices fetch (with all fixes)
  useEffect(() => {
    ...
    const updatePrices = async () => {
      ...
      try {
        ...
        // ---------- ISSUE #6 FIX: CoinGecko batch fetch validation ----------
        if (normalCoins.length > 0) {
          ...
          for (const batch of batches) {
            ...
            if (res && res.ok) {
              const data = await res.json();
              // ✅ VALIDATE: data is object with coin entries
              if (!data || typeof data !== 'object') {
                console.warn('Invalid CoinGecko price data');
                continue;
              }
              cgJson = { ...cgJson, ...data };
            }
        ...
        // ---------- ISSUE #7 FIX: GeckoTerminal / DexScreener with validation ----------
        let customApiJson = {};
          ...
          try {
              ...
              if (gtRes && gtRes.ok) {
                  ...
                  if (!gtData || !gtData.data || !gtData.data.attributes) {
                      console.warn(`Invalid GeckoTerminal response for ${coin.symbol}`);
                      return;
                  }
                  const price = parseFloat(gtData.data.attributes.price_usd);
                  const change = parseFloat(gtData.data.attributes.price_change_percentage?.h24 || 0);
                  // ✅ VALIDATE: numbers are finite
                  if (!Number.isFinite(price) || price <= 0) {
                      console.warn(`Invalid price for ${coin.symbol}`);
                      return;
                  }
        ...
        if (isMounted) {
          ...
          await Promise.all(coinsToUpdate.map(async (id) => {
            ...
            if (customApiJson[id] && customApiJson[id].usd > 0) {
              ...
              // ---------- ISSUE #8 FIX: Binance fallback with validation ----------
              try {
                const bRes = await fetch(`https://api.binance.com/api/v3/ticker/24hr?symbol=${upperSym}USDT`);
                // ✅ CHECK: HTTP status
                if (!bRes.ok) {
                  console.warn(`Binance API error for ${upperSym}: ${bRes.status}`);
                  return;
                }
                ...
                if (bData.code && bData.code < 0) {
                  throw new Error(`Binance error: ${bData.msg}`);
                }
                const priceValue = parseFloat(bData.lastPrice);
                const changeValue = parseFloat(bData.priceChangePercent);
                // ✅ VALIDATE: numbers are finite
                if (!Number.isFinite(priceValue) || priceValue <= 0) {
                  throw new Error(`Invalid price: ${bData.lastPrice}`);
                }
                price = priceValue;
  ...
  const handleAutoFetchDetails = async () => {
    ...
    try {
      if (fetchMode === 'id') {
        ...
        const data = await res.json();
        // ---------- ISSUE #9 FIX: validate required fields ----------
        if (!data || !data.symbol || !data.name) {
          setFetchError("CoinGecko returned invalid data. Try another ID.");
          setIsFetchingData(false); setFetchStatus(''); return;
        }
        ...
        // ---------- ISSUE #10 FIX: validate GeckoTerminal structure ----------
        if (!json || !json.data || !json.data.attributes) {
          setFetchError("Invalid response from GeckoTerminal. Enter details manually.");
          setIsFetchingData(false); setFetchStatus(''); return;
        }
        const data = json.data.attributes;
        if (!data.symbol || !data.name) {
          setFetchError("Incomplete token data. Enter details manually.");
          setIsFetchingData(false); setFetchStatus(''); return;
        }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3 sm:gap-5">
          {showCoinSkeleton ? (
            ...
            currentListToDisplay.map((coin) => {
              ...
              const isPriceLoading = !livePrice && activeCoins.includes(coin.id);
...
export default CryptoManagerContent;