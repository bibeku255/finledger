...
import { auth } from '../../firebase/firebaseConfig'; // ✅ Imported auth
import { sendEmailVerification, signOut } from 'firebase/auth'; // ✅ Imported verification & signOut
...
const Signup = () => {
  ...
  const handleEmailSignup = async (e) => {
    ...
    // Security Check 2: Domain Validator
    ...
    try {
      ...
      // 3. Instantly log them out so they must verify before accessing the dashboard
      await signOut(auth);
      ...
      <div className="min-h-[80vh] flex items-center justify-center py-10 px-4 animate-in fade-in zoom-in-95 duration-700">
        <div className="w-full max-w-md text-center p-10 rounded-[3rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-2xl relative">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 leading-relaxed text-sm">
            We have sent a verification link to <span className="text-blue-600 font-bold underline">{email}</span>. Please verify your account to access the dashboard.
          </p>
