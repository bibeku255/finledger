...
const SecureNotes = () => {
  ...
  // 🔒 VAULT LOCKED SCREEN - Premium Design
  if (!isVaultUnlocked) {
    return (
        ...
        <div className="w-full max-w-md">
          ...
          <div className="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl p-8 md:p-10 border border-slate-200/50 dark:border-slate-800/50 text-center animate-in zoom-in-95 duration-500">
            ...
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-8 max-w-sm mx-auto leading-relaxed">
              Your notes are encrypted. Enter your 6-digit Master PIN to unlock the vault.
            </p>
  ...
  // 🔓 VAULT DASHBOARD - Premium Design
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
            ...
            {filteredNotes.map((note, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4 sm:mb-5 pt-1">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      <button 
                        ...
                        className="text-slate-400 hover:text-blue-500 p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors"
                        title="Edit note"
                      >
                  ...
                  {/* Title & Date */}
                  <div className="mb-4">
                    <h3 className="text-base sm:text-lg font-black text-slate-800 dark:text-white leading-tight line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                      {note.title}
                    </h3>
                  ...
                  <div className="mt-auto bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 p-3 sm:p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/50 relative group/content">
                    <div className={`font-mono text-xs sm:text-sm break-all pr-10 transition-all duration-300 ${
                      isRevealed 
                        ...
                        : 'text-slate-400 dark:text-slate-600 blur-[3px] sm:blur-[4px] select-none'
                    }`}>
                    ...
                    <div className="absolute right-2 top-2 flex flex-col gap-1.5">
                      <button 
                        onClick={(e) => { e.stopPropagation(); toggleReveal(note.id); }} 
                        className={`p-2 rounded-xl transition-all shadow-sm ${
                          isRevealed 
                            ? 'bg-blue-500 text-white hover:bg-blue-600' 
                            : 'bg-white dark:bg-slate-800 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10'
                        }`} 
          ...
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-4 rounded-2xl border border-slate-200/50 dark:border-slate-800/50 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <span className="text-[10px] sm:text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest flex items-center gap-1">
      ...
      {/* 🚀 ADD/EDIT NOTE MODAL - Premium Design */}
      {isModalOpen && (
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300 max-h-[90vh]">
            ...
            <form onSubmit={handleSaveNote} className="p-5 sm:p-6 space-y-5 overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-2">
                <label className="text-[10px] sm:text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">
                  Information Type
                </label>
                <div className="grid grid-cols-2 gap-2">
                  {categories.map(cat => (
                    <button 
                      ...
                      className={`p-3 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-xl flex items-center gap-2 transition-all border-2 ${
                        formData.categoryId === cat.id 
                          ? `${cat.bg} ${cat.color} ${cat.border} shadow-md scale-[1.02]` 
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="e.g. Trust Wallet Seed, Netflix Pass" 
                  className="w-full p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-sm text-slate-900 dark:text-white placeholder-slate-400 outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all" 
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] sm:text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1 flex justify-between">
                  ...
                  <span className="text-emerald-500 font-bold">
                    {formatGlobalDate ? formatGlobalDate(formData.date, 'short') : formData.date}
                  </span>
                </label>
                <input 
                  type="date" 
                  ...
                  className="w-full p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all" 
                />
              </div>
              ...
              <button 
                type="submit" 
                disabled={isSaving} 
                className="w-full p-4 rounded-2xl font-black text-white text-sm transition-all active:scale-95 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 shadow-xl shadow-emerald-500/20 disabled:opacity-70 flex justify-center items-center gap-2 mt-2"
              >
                {isSaving ? (
                  <>
