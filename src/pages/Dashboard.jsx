...
const Dashboard = () => {
  ...
  // Legacy Dashboard Hook (used for incomes, expenses, khata)
  const [dashboardData, setDashboardData] = useState({
    bankTotal: 0, cashTotal: 0, onlineTotal: 0,
    cryptoTransactions: [],
    rawIncomes: [], rawExpenses: [],
    incomeTotal: 0, expenseTotal: 0,
    khataReceivables: 0, khataPayables: 0,
  });

  const { loading: dataLoading } = useDashboardData(user, setDashboardData);

  // 🚀 ENTERPRISE MATH ENGINE INTEGRATION
  ...
  // Vault Totals Override (O(1) Fetching if summary exists, else fallback to raw data)
  const finalBankTotal = bankSummary?.totalBalance ?? dashboardData.bankTotal;
  const finalCashTotal = cashSummary?.totalBalance ?? dashboardData.cashTotal;
  const finalOnlineTotal = onlineSummary?.totalBalance ?? dashboardData.onlineTotal;
  ...
  // 🚀 O(1) Crypto Balances Integration
  const unifiedCryptoHoldings = useMemo(() => {
    // Priority 1: Summary Engine
    if (cryptoSummary?.holdings) {
      const holdings = {};
      Object.entries(cryptoSummary.holdings).forEach(([coin, data]) => {
        if (data.total > 0.00000001) holdings[coin.toUpperCase()] = data.total;
      });
      return holdings;
    }
    ...
    const holdings = {};
    (dashboardData.cryptoTransactions || []).forEach(tx => {
      const coin = tx.coin?.toUpperCase();
      if (!coin) return;
      const qty = safeParseFloat(tx.quantity);
      const fee = safeParseFloat(tx.fee || tx.networkFee);
      const totalQty = safeParseFloat(tx.totalQuantity) || (qty + fee);
      if (!holdings[coin]) holdings[coin] = 0;
      const txType = (tx.type || '').toLowerCase();
      if (txType === 'in') holdings[coin] += qty;
      else if (txType === 'out') holdings[coin] -= totalQty;
      else if (txType === 'transfer') holdings[coin] -= fee;
    });
    Object.keys(holdings).forEach(coin => {
      if (holdings[coin] <= 0.00000001) delete holdings[coin];
    });
    return holdings;
  }, [dashboardData.cryptoTransactions, cryptoSummary]);
    ...
    Object.entries(livePrices || {}).forEach(([sym, data]) => {
      if (watchlistSymbols.includes(sym) || (sym === 'USDT' && watchlistSymbols.length > 0)) {
        ...
        newMarketData.push({
          symbol: sym,
          type: 'crypto',
          priceUSD: safeParseFloat(data.priceUSD),
          priceBase: safeParseFloat(data.priceUSD) * fiatRate,
          change: safeParseFloat(data.change),
  ...
  const cryptoTotal = useMemo(() => {
    ...
    return Object.entries(unifiedCryptoHoldings).reduce((sum, [coin, balance]) => {
      const priceUSD = safeParseFloat(livePrices[coin.toUpperCase()]?.priceUSD);
      return sum + (balance * priceUSD * fiatRate);
    }, 0);
  }, [unifiedCryptoHoldings, livePrices, fiatRate]);

  const netWorth = finalBankTotal + finalCashTotal + finalOnlineTotal + cryptoTotal;
  const savingsTotal = dashboardData.incomeTotal - dashboardData.expenseTotal;
  const savingsRate = dashboardData.incomeTotal > 0 ? ((savingsTotal / dashboardData.incomeTotal) * 100).toFixed(1) : 0;
  const monthlyTrend = dashboardData.incomeTotal > dashboardData.expenseTotal ? 'positive' : 'negative';

  const assetAllocation = useMemo(() => {
    return [
      { name: 'Bank Ledger', value: Math.max(0, finalBankTotal) },
      { name: 'Physical Cash', value: Math.max(0, finalCashTotal) },
      { name: 'E-Wallets', value: Math.max(0, finalOnlineTotal) },
      { name: 'Crypto Assets', value: Math.max(0, cryptoTotal) }
    ].filter(asset => asset.value > 0);
  }, [finalBankTotal, finalCashTotal, finalOnlineTotal, cryptoTotal]);

  const memoizedFormatGlobalDate = useCallback((date, type) => {
    return formatGlobalDate ? formatGlobalDate(date, type) : date.toLocaleString('default', { month: 'short', year: 'numeric' });
  }, [formatGlobalDate]);

  const chartCashFlow = useMemo(() => {
    const monthlyData = {};
    dashboardData.rawIncomes.forEach(data => {
      const amt = safeParseFloat(data.finalBaseAmount || data.amount);
      const dateObj = new Date(data.date || new Date());
      const monthName = memoizedFormatGlobalDate(dateObj, 'monthYear');
  ...
  }, [dashboardData.rawIncomes, dashboardData.rawExpenses, memoizedFormatGlobalDate]);

  const recentTransactions = useMemo(() => {
    const inc = dashboardData.rawIncomes.map(i => ({ ...i, txType: 'income', amt: safeParseFloat(i.finalBaseAmount || i.amount) }));
    const exp = dashboardData.rawExpenses.map(e => ({ ...e, txType: 'expense', amt: safeParseFloat(e.finalBaseAmount || e.amount) }));
    return [...inc, ...exp].sort((a, b) => b.timestamp - a.timestamp).slice(0, 5);
  }, [dashboardData.rawIncomes, dashboardData.rawExpenses]);

  const categoryBreakdown = useMemo(() => {
    const now = new Date();
    const thisMonthPrefix = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    const expCat = {}; const incCat = {};
    let eTotal = 0, iTotal = 0;

    dashboardData.rawExpenses.forEach(e => {
      if (e.date?.startsWith(thisMonthPrefix)) {
        const amt = safeParseFloat(e.finalBaseAmount || e.amount);
        expCat[e.category] = (expCat[e.category] || 0) + amt;
        eTotal += amt;
      }
    });

    dashboardData.rawIncomes.forEach(i => {
      if (i.date?.startsWith(thisMonthPrefix)) {
        const amt = safeParseFloat(i.finalBaseAmount || i.amount);
        incCat[i.category] = (incCat[i.category] || 0) + amt;
        iTotal += amt;
      }
    });

    const expArr = Object.keys(expCat).map(k => ({ name: k, value: expCat[k], percent: eTotal > 0 ? (expCat[k]/eTotal)*100 : 0 })).sort((a,b)=>b.value-a.value).slice(0, 4);
    const incArr = Object.keys(incCat).map(k => ({ name: k, value: incCat[k], percent: iTotal > 0 ? (incCat[k]/iTotal)*100 : 0 })).sort((a,b)=>b.value-a.value).slice(0, 4);

    return { expenses: expArr, incomes: incArr, eTotal, iTotal };
  }, [dashboardData.rawExpenses, dashboardData.rawIncomes]);
  ...
  // ✅ Show Loading State when either hook is processing
  if (dataLoading || summariesLoading) {
    return (
    ...
    <div className="w-full h-auto pb-10">
      <div className="w-full max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 px-2 md:px-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              <div className="flex items-start gap-3 cursor-pointer group" onClick={() => navigate('/dashboard/accounts/bank')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] sm:text-sm font-bold text-white break-words leading-tight">{currencySymbol}{finalBankTotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 cursor-pointer group" onClick={() => navigate('/dashboard/accounts/cash')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] sm:text-sm font-bold text-white break-words leading-tight">{currencySymbol}{finalCashTotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 cursor-pointer group" onClick={() => navigate('/dashboard/accounts/online')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] sm:text-sm font-bold text-white break-words leading-tight">{currencySymbol}{finalOnlineTotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
              </div>
            ...
            <div className="mt-6 pt-5 border-t border-white/10 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              <div className="flex items-start gap-3 cursor-pointer group" onClick={() => navigate('/dashboard/parties')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] sm:text-sm font-bold text-white break-words leading-tight">{currencySymbol}{dashboardData.khataReceivables.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 cursor-pointer group" onClick={() => navigate('/dashboard/parties')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] sm:text-sm font-bold text-white break-words leading-tight">{currencySymbol}{dashboardData.khataPayables.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <StatCard title="Total Income" value={`${currencySymbol}${dashboardData.incomeTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}`} icon={HiOutlineTrendingUp} gradient={GRADIENTS.income} trend={5.2} subtitle="Lifetime earnings" onClick={() => navigate('/dashboard/income')} />
          <StatCard title="Total Expenses" value={`${currencySymbol}${dashboardData.expenseTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}`} icon={HiOutlineTrendingDown} gradient={GRADIENTS.expense} trend={-2.1} subtitle="Lifetime spending" onClick={() => navigate('/dashboard/expense')} />
