...
import { 
  FaTwitter, FaDiscord, FaLinkedin, FaGithub 
} from 'react-icons/fa';
  ...
  return (
    <footer className="bg-slate-950 pt-16 md:pt-20 pb-8 md:pb-10 border-t border-slate-900 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 relative z-10">
          ...
          <form onSubmit={handleSubscribe} className="w-full lg:w-auto flex-1 max-w-md relative mt-2 lg:mt-0">
            <div className="relative flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-0">
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-slate-800 text-white px-4 sm:pl-12 sm:pr-36 py-3.5 md:py-4 rounded-xl sm:rounded-2xl outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium placeholder:text-slate-600 text-sm md:text-base"
              />
              <button 
                type="submit" 
                className="sm:absolute sm:right-2 sm:top-2 sm:bottom-2 w-full sm:w-auto bg-blue-600 hover:bg-blue-500 text-white px-5 py-3.5 sm:py-0 rounded-xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 mt-2 sm:mt-0"
              >
                {isSubscribed ? 'Done!' : 'Subscribe'}
              </button>
            </div>
            {isSubscribed && (
              <p className="absolute -bottom-6 left-1/2 -translate-x-1/2 sm:translate-x-0 sm:left-2 text-[10px] font-bold text-emerald-500 uppercase tracking-widest text-center sm:text-left w-full sm:w-auto mt-1 sm:mt-0">
        ...
        {/* Mobile par 1 column then Brand, uske baad 2 columns. Tablet par 2, Desktop par 4 */}
          ...
          <div className="lg:col-span-1 flex flex-col items-center text-center lg:items-start lg:text-left">
            <Link to="/" className="flex items-center gap-3 mb-4 md:mb-6">
              <img 
                src={logo} 
                alt="Finledger Logo" 
                className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover border border-slate-700 shadow-lg bg-slate-800 p-0.5" 
              />
            ...
            <div className="flex gap-3 justify-center lg:justify-start">
              <a href="#" className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-blue-400 hover:border-blue-500/50 hover:bg-blue-500/10 transition-all">
                <FaTwitter size={16} className="md:text-lg" />
              </a>
              <a href="#" className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-indigo-400 hover:border-indigo-500/50 hover:bg-indigo-500/10 transition-all">
                <FaDiscord size={16} className="md:text-lg" />
              </a>
              <a href="#" className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-blue-500 hover:border-blue-600/50 hover:bg-blue-600/10 transition-all">
                <FaLinkedin size={16} className="md:text-lg" />
              </a>
              <a href="#" className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white hover:border-slate-600 hover:bg-slate-800 transition-all">
