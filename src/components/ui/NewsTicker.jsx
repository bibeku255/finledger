...
import { 
  ...
  Zap, ArrowRight, Activity, Bell, BellOff
} from 'lucide-react';
...
const NewsTicker = () => {
  ...
  // 🚀 Fetch Custom Coins to ensure Contract Addresses match Wallet
  useEffect(() => {
  ...
  // Combine Global Cryptos + Custom User Coins
  ...
  const triggerMarketAlert = async (symbol, changePercent) => {
    if (!user || !showAlerts || Math.abs(changePercent) < 5) return;
    const tzOffset = (new Date()).getTimezoneOffset() * 60000;
    const localISOTime = (new Date(Date.now() - tzOffset)).toISOString().split('T')[0];
    const uniqueNotifId = `MARKET_ALERT_${symbol}_${localISOTime}`;
    ...
    try {
      ...
      if (notifSnap.empty) {
        ...
        await addDoc(collection(db, "users", user.uid, "notifications"), {
          uniqueId: uniqueNotifId, title: isPump ? `🚀 Market Pump!` : `📉 Market Dump!`,
          message: `${symbol} is ${isPump ? 'up' : 'down'} by ${Math.abs(changePercent).toFixed(1)}% in the last 24h.`,
          type: 'market_alert', isRead: false, timestamp: new Date().getTime(), link: '/dashboard/crypto/wallet'
        });
      }
    } catch (error) {}
  };

  // 🚀 5-LAYER FETCH ENGINE (Exactly same as CryptoWallet & SmartConverter)
  const fetchMarketData = useCallback(async () => {
    ...
    if (hasCrypto) {
      ...
      // 2. CoinGecko API for standard IDs
      try {
      ...
      // 3. DEX APIs for Custom Contract Coins
      let customApiJson = {};
      await Promise.all(contractCoins.map(async (coin) => {
        try {
            ...
            if (!customApiJson[coin.id || coin.symbol.toLowerCase()] && coin.network) {
      ...
      const newCryptoData = await Promise.all(fullDatabase.map(async (coin) => {
        ...
        if (changePercent !== 0) triggerMarketAlert(coin.symbol, changePercent);
    ...
    if (hasForex) {
      setForexData(selectedFiats.filter(fiat => fiat !== baseCurrency).map(fiat => ({
          pair: `${fiat}/${baseCurrency}`, code: fiat,
          rate: (usdToBase / parseFloat(fiatRates[fiat])) || 0,
          changePercent: (Math.random() * 0.4 - 0.2), 
          flagId: fiatFlagMap[fiat] || 'un', hasAlert: false
      })));
    }
    ...
    <div className="w-full bg-gradient-to-b from-slate-950 to-slate-900 border-b border-slate-800/50 select-none overflow-hidden sticky top-0 z-30 shadow-xl shadow-black/20">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gradient-to-r from-slate-900/95 via-slate-900/90 to-slate-900/95 backdrop-blur-xl border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className={`hidden xs:flex items-center gap-2 px-2.5 py-1 rounded-full text-[9px] uppercase tracking-widest font-black transition-all duration-300 ${isRefreshing ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-slate-800/50 text-slate-500 border border-slate-700/30'}`}>
            <RefreshCcw size={10} className={isRefreshing ? 'animate-spin' : ''} />
            <span>{isRefreshing ? 'UPDATING' : 'LIVE'}</span>
            {!isRefreshing && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse ml-0.5" />}
          </div>
      ...
      <div className="relative overflow-hidden py-2 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        ...
        {currentData.length > 0 ? (
          ...
          <div className="text-center py-2 text-xs font-black text-slate-500 animate-pulse">Fetching {activeTicker} data...</div>
        )}
      </div>
