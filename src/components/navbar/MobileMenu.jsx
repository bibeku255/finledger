...
const LegalNavItem = ({ item, onClose }) => (
  ...
  >
    {({ isActive }) => (
      <>
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform ${isActive ? 'bg-blue-100 dark:bg-blue-500/20' : 'bg-slate-100 dark:bg-slate-800'
          }`}>
          {item.icon}
        </div>
        <span>{item.name}</span>
...
const MobileMenu = ({ isOpen, onClose }) => {
  const { user, dbData, logout, baseCurrency, avatar, displayName, isAdmin } = useAuth();
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`absolute inset-0 z-40 bg-gradient-to-br from-slate-950/90 via-slate-900/85 to-slate-950/90 backdrop-blur-xl transition-all duration-500 md:hidden ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'
          }`}
        onClick={onClose}
      ...
      {/* Drawer */}
      <aside
        className={`absolute md:hidden top-0 right-0 bottom-0 z-50 w-[75%] max-w-[300px] bg-white/98 dark:bg-slate-950/98 backdrop-blur-2xl border-l border-slate-200/30 dark:border-slate-800/50 shadow-2xl transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none'
          }`}
      >
        {/* Top gradient bar */}
        ...
        <nav className="flex-1 overflow-y-auto px-3 pt-4 pb-2 space-y-1 custom-scrollbar overscroll-contain">
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <div className={`relative shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 group-hover:scale-110 ${isActive
                  ...
                  {item.featured && item.badge && (
                    <span className={`shrink-0 px-1.5 py-0.5 text-[8px] font-black rounded-md border ${item.isAlert
                  ...
                  <HiOutlineArrowRight size={14} className={`shrink-0 transition-all duration-300 ${isActive ? 'opacity-100 translate-x-0 text-blue-500' : 'opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 text-slate-400'
                    }`} />
                </>
              )}
            </NavLink>
          ...
          <div className="pt-3 mt-3 border-t border-slate-200/60 dark:border-slate-800/50">
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-[10px] font-black text-slate-500 uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all group border ${isMoreOpen ? 'border-blue-200 dark:border-blue-500/30 bg-blue-50/50 dark:bg-blue-500/5' : 'border-transparent hover:border-slate-200/60 dark:hover:border-slate-700/50'
                }`}
            >
              <span className="flex items-center gap-2.5">
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all duration-300 ${isMoreOpen
              ...
              <div className="flex items-center gap-1.5">
                <span className={`text-[8px] px-1.5 py-0.5 rounded-md border transition-all ${isMoreOpen
            ...
            <div className={`grid transition-all duration-400 ease-in-out ${isMoreOpen ? 'grid-rows-[1fr] opacity-100 mt-1.5' : 'grid-rows-[0fr] opacity-0'
              }`}>
            ...
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="group p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/50 hover:border-blue-400/60 dark:hover:border-blue-500/50 transition-all active:scale-[0.98] shadow-sm text-left overflow-hidden relative"
                >
                ...
                <button
                  ...
                  className="group p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/50 hover:border-indigo-400/60 dark:hover:border-indigo-500/50 transition-all active:scale-[0.98] shadow-sm text-left overflow-hidden relative"
                >
              ...
              {isAdmin && (
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-[11px] uppercase tracking-widest shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-1"
                >
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => handleNavigation('/profile')}
                  className="flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm transition-all active:scale-[0.98] hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center justify-center gap-1.5"
                >
                  <HiOutlineCog size={14} /> Config
                </button>
                <button
                  onClick={() => { logout(); onClose(); navigate('/'); }}
                  className="flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 shadow-sm transition-all active:scale-[0.98] hover:bg-rose-100 dark:hover:bg-rose-500/20 flex items-center justify-center gap-1.5"
                >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <button
                  ...
                  className="w-full py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm active:scale-[0.98] transition-all hover:bg-slate-50 dark:hover:bg-slate-700"
                >
                  Sign In
                </button>
                <button
                  onClick={() => handleNavigation('/signup')}
                  className="w-full py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-white bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-500/30 active:scale-[0.98] transition-all flex items-center justify-center gap-1.5"
                >
